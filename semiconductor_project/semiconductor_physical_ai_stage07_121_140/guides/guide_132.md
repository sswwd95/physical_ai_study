...
## 1. 학습 목표
RandomizedSearchCV로 Gradient Boosting 설정을 효율적으로 탐색합니다.
...
python examples\ex132_randomized_search_gradient_boosting.py
```

## 4. 예상 결과
Gradient Boosting의 유망한 하이퍼파라미터 조합이 출력됩니다.
...
| 17 | `from sklearn.model_selection import RandomizedSearchCV, train_test_split` | 필요한 라이브러리나 모델을 불러옵니다. |
...
| 44 | `    ("cat", OneHotEncoder(handle_unknown="ignore", sparse_output=False), categorical_features),` | 계산 결과나 설정값을 변수에 저장합니다. |
...
| 49 | `    ("classifier", HistGradientBoostingClassifier(random_state=42)),` | 계산 결과나 설정값을 변수에 저장합니다. |
| 50 | `])` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
| 51 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 52 | `param_distributions = {` | 계산 결과나 설정값을 변수에 저장합니다. |
| 53 | `    "classifier__learning_rate": [0.03, 0.05, 0.08, 0.12],` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
| 54 | `    "classifier__max_iter": [100, 200, 300],` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
| 55 | `    "classifier__max_depth": [3, 5, 7, None],` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
| 56 | `    "classifier__l2_regularization": [0.0, 0.1, 1.0],` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
| 57 | `}` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
| 58 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 59 | `search = RandomizedSearchCV(` | 일부 하이퍼파라미터 조합을 무작위로 탐색합니다. |
| 60 | `    pipeline,` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
| 61 | `    param_distributions=param_distributions,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 62 | `    n_iter=8,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 63 | `    scoring="f1_macro",` | 계산 결과나 설정값을 변수에 저장합니다. |
| 64 | `    cv=3,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 65 | `    random_state=42,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 66 | `    n_jobs=-1,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 67 | `)` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
| 68 | `search.fit(x_train, y_train)` | 학습 데이터로 전처리기와 모델을 학습합니다. |
| 69 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 70 | `print("최적 파라미터:", search.best_params_)` | 실행 결과를 콘솔에 출력합니다. |
| 71 | `print("최적 CV macro F1:", round(search.best_score_, 4))` | 실행 결과를 콘솔에 출력합니다. |
