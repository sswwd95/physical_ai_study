...
## 1. 학습 목표
Random Forest로 안정적인 다중 불량 유형 분류를 수행합니다.
...
python examples\ex127_multiclass_random_forest.py
```

## 4. 예상 결과
클래스별 precision·recall·F1이 포함된 Random Forest 보고서가 출력됩니다.
...
| 53 | `            n_estimators=400,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 54 | `            max_depth=10,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 55 | `            min_samples_leaf=4,` | 계산 결과나 설정값을 변수에 저장합니다. |
...
| 63 | `model.fit(x_train, y_train)` | 학습 데이터로 전처리기와 모델을 학습합니다. |
| 64 | `y_pred = model.predict(x_test)` | 학습된 모델로 불량 유형을 예측합니다. |
| 65 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 66 | `print(classification_report(y_test, y_pred, zero_division=0))` | 클래스별 정밀도·재현율·F1을 요약합니다. |
