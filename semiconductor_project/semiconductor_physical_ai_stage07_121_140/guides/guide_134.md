...
## 1. 학습 목표
클래스별 precision·recall·F1과 support를 표 형태로 저장합니다.
...
python examples\ex134_classwise_metrics.py
```

## 4. 예상 결과
클래스별 성능과 support가 표로 저장됩니다.
...
| 17 | `from sklearn.metrics import classification_report` | 필요한 라이브러리나 모델을 불러옵니다. |
...
| 64 | `report = classification_report(` | 클래스별 정밀도·재현율·F1을 요약합니다. |
| 65 | `    y_test,` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
| 66 | `    y_pred,` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
| 67 | `    output_dict=True,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 68 | `    zero_division=0,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 69 | `)` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
| 70 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 71 | `report_df = pd.DataFrame(report).T` | 계산 결과나 설정값을 변수에 저장합니다. |
| 72 | `print(report_df.round(4))` | 실행 결과를 콘솔에 출력합니다. |
| 73 | `report_df.to_csv(` | 결과를 CSV 파일로 저장합니다. |
| 74 | `    OUTPUT_DIR / "ex134_classwise_metrics.csv",` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
| 75 | `    encoding="utf-8-sig",` | 계산 결과나 설정값을 변수에 저장합니다. |
| 76 | `)` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
