...
python examples\ex245_failure_probability_by_cycle.py
```
...
| 22 | `for cycle in [80,100,120,150]:` | 여러 장비·그룹·정책을 반복 분석합니다. |
| 23 | `    p=1-np.exp(-(cycle/b)**a)` | 계산 결과나 모형 파라미터를 변수에 저장합니다. |
| 24 | `    print(cycle,"주기 이전 고장확률:",round(p.mean(),4))` | 결과를 콘솔에 출력합니다. |
