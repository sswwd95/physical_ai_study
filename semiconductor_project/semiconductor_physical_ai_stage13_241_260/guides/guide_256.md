...
python examples\ex256_prior_sensitivity_lifetime.py
```
...
| 17 | `rows=[]` | 계산 결과나 모형 파라미터를 변수에 저장합니다. |
| 18 | `for prior_scale in [80,150,250]:` | 여러 장비·그룹·정책을 반복 분석합니다. |
| 19 | `    with pm.Model() as model:` | PyMC 확률모형 범위를 시작합니다. |
| 20 | `        alpha=pm.HalfNormal("alpha",3); beta=pm.HalfNormal("beta",prior_scale)` | 0보다 큰 분산·스케일 파라미터를 정의합니다. |
| 21 | `        pm.Weibull("life",alpha=alpha,beta=beta,observed=obs)` | 설비 수명을 Weibull 분포로 모델링합니다. |
| 22 | `        idata=pm.sample(600,tune=600,chains=2,cores=1,random_seed=42,progressbar=False)` | MCMC로 사후분포 표본을 추출합니다. |
| 23 | `    rows.append({"prior_scale":prior_scale,"beta_mean":float(idata.posterior["beta"].mean())})` | 베이지안 수명·고장확률·RUL 분석 단계를 수행합니다. |
| 24 | `out=pd.DataFrame(rows); print(out.round(3)); out.to_csv(OUTPUT_DIR/"ex256_prior_sensitivity.csv",index=False,encoding="utf-8-sig")` | 분석 결과를 CSV로 저장합니다. |
