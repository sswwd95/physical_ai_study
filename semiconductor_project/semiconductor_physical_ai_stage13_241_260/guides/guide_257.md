...
python examples\ex257_loo_lifetime_comparison.py
```
...
| 17 | `models={}` | 계산 결과나 모형 파라미터를 변수에 저장합니다. |
| 18 | `with pm.Model() as exp_model:` | PyMC 확률모형 범위를 시작합니다. |
| 19 | `    rate=pm.Exponential("rate",1/120); pm.Exponential("life",lam=rate,observed=obs)` | 양수 파라미터에 지수분포 사전분포를 사용합니다. |
| 20 | `    models["exponential"]=pm.sample(600,tune=600,chains=2,cores=1,random_seed=42,progressbar=False,idata_kwargs={"log_likelihood":True})` | MCMC로 사후분포 표본을 추출합니다. |
| 21 | `with pm.Model() as weib_model:` | PyMC 확률모형 범위를 시작합니다. |
| 22 | `    alpha=pm.HalfNormal("alpha",3); beta=pm.HalfNormal("beta",150); pm.Weibull("life",alpha=alpha,beta=beta,observed=obs)` | 설비 수명을 Weibull 분포로 모델링합니다. |
| 23 | `    models["weibull"]=pm.sample(600,tune=600,chains=2,cores=1,random_seed=42,progressbar=False,idata_kwargs={"log_likelihood":True})` | MCMC로 사후분포 표본을 추출합니다. |
| 24 | `print(az.compare(models))` | 결과를 콘솔에 출력합니다. |
