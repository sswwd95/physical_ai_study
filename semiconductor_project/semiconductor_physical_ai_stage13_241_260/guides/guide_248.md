...
python examples\ex248_chamber_type_effect.py
```
...
| 16 | `codes,types=pd.factorize(life_df["chamber_type"],sort=True)` | 계산 결과나 모형 파라미터를 변수에 저장합니다. |
| 17 | `y=np.log(life_df["observed_cycles"].to_numpy())` | 계산 결과나 모형 파라미터를 변수에 저장합니다. |
| 18 | `with pm.Model(coords={"type":types}) as model:` | PyMC 확률모형 범위를 시작합니다. |
| 19 | `    a=pm.Normal("a",np.log(120),1)` | 회귀계수나 그룹 효과의 정규 사전분포를 정의합니다. |
| 20 | `    effect=pm.Normal("effect",0,.5,dims="type")` | 회귀계수나 그룹 효과의 정규 사전분포를 정의합니다. |
| 21 | `    sigma=pm.HalfNormal("sigma",.5)` | 0보다 큰 분산·스케일 파라미터를 정의합니다. |
| 22 | `    pm.Normal("log_life",a+effect[codes],sigma,observed=y)` | 회귀계수나 그룹 효과의 정규 사전분포를 정의합니다. |
| 23 | `    idata=pm.sample(1000,tune=1000,chains=2,cores=1,random_seed=42,progressbar=False)` | MCMC로 사후분포 표본을 추출합니다. |
| 24 | `print(az.summary(idata,var_names=["effect"],hdi_prob=.94))` | 사후요약과 R-hat·ESS를 계산합니다. |
