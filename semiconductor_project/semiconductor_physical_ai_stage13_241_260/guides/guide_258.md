...
python examples\ex258_mcmc_diagnostics_lifetime.py
```
...
| 19 | `    idata=pm.sample(800,tune=800,chains=2,cores=1,target_accept=.9,random_seed=42,progressbar=False)` | MCMC로 사후분포 표본을 추출합니다. |
| 20 | `summary=az.summary(idata,var_names=["alpha","beta"]); div=int(idata.sample_stats["diverging"].sum())` | 사후요약과 R-hat·ESS를 계산합니다. |
| 21 | `print(summary); print("divergence:",div)` | 결과를 콘솔에 출력합니다. |
| 22 | `summary.assign(divergence_count=div).to_csv(OUTPUT_DIR/"ex258_mcmc_diagnostics.csv",encoding="utf-8-sig")` | 분석 결과를 CSV로 저장합니다. |
