...
python examples\ex247_median_lifetime_hdi.py
```
...
| 20 | `    median=pm.Deterministic("median_lifetime",beta*(np.log(2))**(1/alpha))` | 계산 결과나 모형 파라미터를 변수에 저장합니다. |
| 21 | `    idata=pm.sample(900,tune=900,chains=2,cores=1,random_seed=42,progressbar=False)` | MCMC로 사후분포 표본을 추출합니다. |
| 22 | `print(az.summary(idata,var_names=["median_lifetime"],hdi_prob=.94))` | 사후요약과 R-hat·ESS를 계산합니다. |
