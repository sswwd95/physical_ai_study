...
python examples\ex183_target_exceedance_probability.py
```
...
| 13 | `n = int(defect_df["wafer_count"].sum())` | 계산 결과나 설정값을 변수에 저장합니다. |
| 14 | `k = int(defect_df["defect_count"].sum())` | 계산 결과나 설정값을 변수에 저장합니다. |
| 15 | `with pm.Model() as model:` | PyMC 확률모형 범위를 시작합니다. |
| 16 | `    p = pm.Beta("p", 1, 1)` | 0~1 확률용 베타 사전분포를 정의합니다. |
| 17 | `    pm.Binomial("d", n=n, p=p, observed=k)` | 불량 개수를 이항분포 관측값으로 연결합니다. |
...
| 20 | `s = idata.posterior["p"].values.ravel()` | 계산 결과나 설정값을 변수에 저장합니다. |
| 21 | `print("P(p>0.04):", round((s>0.04).mean(),4))` | 계산 결과를 콘솔에 출력합니다. |
| 22 | `print("P(p>0.05):", round((s>0.05).mean(),4))` | 계산 결과를 콘솔에 출력합니다. |
