...
python examples\ex188_hierarchical_chamber_model.py
```
...
| 13 | `codes,chs=pd.factorize(defect_df["chamber_id"],sort=True)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 14 | `with pm.Model(coords={"chamber":chs}) as model:` | PyMC 확률모형 범위를 시작합니다. |
| 15 | `    a=pm.Normal("a",-3,1); tau=pm.HalfNormal("tau",1); z=pm.Normal("z",0,1,dims="chamber")` | 계산 결과나 설정값을 변수에 저장합니다. |
...
| 18 | `    idata=pm.sample(900,tune=900,chains=2,cores=1,target_accept=.9,random_seed=42,progressbar=False)` | MCMC로 사후분포 표본을 추출합니다. |
| 19 | `print(az.summary(idata,var_names=["a","tau","z"]))` | 사후요약과 MCMC 진단값을 계산합니다. |
