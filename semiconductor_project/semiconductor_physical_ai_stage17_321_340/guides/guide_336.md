...
python examples\ex336_credible_interval_fusion.py
```
...
| 14 | `data=sensor_df[["temp_sensor_a_c","temp_sensor_b_c"]].dropna().iloc[:120]` | 계산 결과나 모형 파라미터를 변수에 저장합니다. |
...
| 20 | `    pm.Normal("a",latent+ba,sa,observed=a,dims="obs")` | 센서 바이어스·참값·관측값의 정규분포를 정의합니다. |
| 21 | `    pm.Normal("b",latent+bb,sb,observed=b,dims="obs")` | 센서 바이어스·참값·관측값의 정규분포를 정의합니다. |
| 22 | `    idata=pm.sample(700,tune=700,chains=2,cores=1,random_seed=42,progressbar=False)` | MCMC로 사후분포 표본을 추출합니다. |
| 23 | `arr=idata.posterior["latent"].stack(sample=("chain","draw")).values` | 계산 결과나 모형 파라미터를 변수에 저장합니다. |
| 24 | `h=az.hdi(arr.T,hdi_prob=.94)` | 최고밀도구간을 계산합니다. |
| 25 | `out=pd.DataFrame({"fused_mean":arr.mean(axis=1),"hdi_low":h[:,0],"hdi_high":h[:,1]})` | 계산 결과나 모형 파라미터를 변수에 저장합니다. |
| 26 | `out.to_csv(OUTPUT_DIR/"ex336_credible_interval_fusion.csv",index=False,encoding="utf-8-sig")` | 결과를 CSV 파일로 저장합니다. |
| 27 | `print(out.head(10).round(3))` | 결과를 콘솔에 출력합니다. |
