...
candidate_df["utility"]=(
    0.55*zscore(candidate_df["uniformity_percent"])
    -0.30*zscore(candidate_df["defect_rate"])
    -0.15*zscore(candidate_df["cycle_time_min"])
)
out=candidate_df.sort_values("utility",ascending=False).head(30)
print(out.round(4))
out.to_csv(OUTPUT_DIR/"ex288_weighted_utility.csv",index=False,encoding="utf-8-sig")
