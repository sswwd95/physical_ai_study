# 실습 293 — lower_confidence_bound

## 1. 학습 목표
보수적 신뢰하한으로 후보를 선택합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
95% 보수적 신뢰하한이 높은 후보를 선택하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage15
python examples\ex293_lower_confidence_bound.py
```
...
| 41 | `for seed in range(20):` | 여러 후보나 가중치 조합을 반복 계산합니다. |
...
| 46 | `candidate_df["mean"]=mat.mean(0)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 47 | `candidate_df["lcb_95"]=mat.mean(0)-1.64*mat.std(0)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 48 | `out=candidate_df.sort_values("lcb_95",ascending=False).head(25)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 49 | `print(out.round(4))` | 결과를 콘솔에 출력합니다. |
| 50 | `out.to_csv(OUTPUT_DIR/"ex293_lcb_selection.csv",index=False,encoding="utf-8-sig")` | 추천 결과를 CSV로 저장합니다. |
