# 실습 290 — robust_condition_score

## 1. 학습 목표
예측 변동을 고려한 강건 점수를 계산합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
앙상블 평균-1.64표준편차로 강건 점수를 계산하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage15
python examples\ex290_robust_condition_score.py
```
...
| 40 | `predictions=[]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 41 | `for seed in range(15):` | 여러 후보나 가중치 조합을 반복 계산합니다. |
| 42 | `    m=make_model(seed)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 43 | `    m.fit(history_df[features],history_df["uniformity_percent"])` | 과거 공정 데이터로 대리모델을 학습합니다. |
| 44 | `    predictions.append(m.predict(candidate_df[features]))` | 후보 공정 조건의 품질·불량률·시간을 예측합니다. |
| 45 | `mat=np.vstack(predictions)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 46 | `candidate_df["mean_uniformity"]=mat.mean(0)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 47 | `candidate_df["std_uniformity"]=mat.std(0)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 48 | `candidate_df["robust_score"]=candidate_df["mean_uniformity"]-1.64*candidate_df["std_uniformity"]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 49 | `out=candidate_df.sort_values("robust_score",ascending=False).head(25)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 50 | `print(out.round(4))` | 결과를 콘솔에 출력합니다. |
| 51 | `out.to_csv(OUTPUT_DIR/"ex290_robust_scores.csv",index=False,encoding="utf-8-sig")` | 추천 결과를 CSV로 저장합니다. |
