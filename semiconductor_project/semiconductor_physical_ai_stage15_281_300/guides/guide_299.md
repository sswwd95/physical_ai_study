# 실습 299 — confirmation_experiment_plan

## 1. 학습 목표
추천 조건의 확인 실험 계획을 생성합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
상위 5개 조건을 3회 반복하고 실행순서를 무작위화하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage15
python examples\ex299_confirmation_experiment_plan.py
```
...
| 42 | `candidate_df["predicted_uniformity"]=m.predict(candidate_df[features])` | 후보 공정 조건의 품질·불량률·시간을 예측합니다. |
| 43 | `top=candidate_df.sort_values("predicted_uniformity",ascending=False).head(5).copy()` | 계산 결과나 설정값을 변수에 저장합니다. |
| 44 | `replicated=pd.concat([top.assign(replicate=i+1) for i in range(3)],ignore_index=True)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 45 | `replicated["random_order"]=np.random.default_rng(42).permutation(np.arange(1,len(replicated)+1))` | 계산 결과나 설정값을 변수에 저장합니다. |
| 46 | `replicated=replicated.sort_values("random_order")` | 계산 결과나 설정값을 변수에 저장합니다. |
| 47 | `print(replicated)` | 결과를 콘솔에 출력합니다. |
| 48 | `replicated.to_csv(OUTPUT_DIR/"ex299_confirmation_plan.csv",index=False,encoding="utf-8-sig")` | 추천 결과를 CSV로 저장합니다. |
