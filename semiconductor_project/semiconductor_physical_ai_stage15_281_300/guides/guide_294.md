# 실습 294 — safe_optimization

## 1. 학습 목표
불량률 제약을 만족할 확률로 안전 후보를 선택합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
P(불량률<4%)≥0.9 조건에서 균일도 최고 후보를 선택하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage15
python examples\ex294_safe_optimization.py
```
...
| 40 | `defect_predictions=[]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 41 | `uniformity_predictions=[]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 42 | `for seed in range(20):` | 여러 후보나 가중치 조합을 반복 계산합니다. |
| 43 | `    md=make_model(seed); md.fit(history_df[features],history_df["defect_rate"])` | 과거 공정 데이터로 대리모델을 학습합니다. |
| 44 | `    mu=make_model(seed+100); mu.fit(history_df[features],history_df["uniformity_percent"])` | 과거 공정 데이터로 대리모델을 학습합니다. |
| 45 | `    defect_predictions.append(md.predict(candidate_df[features]))` | 후보 공정 조건의 품질·불량률·시간을 예측합니다. |
| 46 | `    uniformity_predictions.append(mu.predict(candidate_df[features]))` | 후보 공정 조건의 품질·불량률·시간을 예측합니다. |
| 47 | `dmat=np.vstack(defect_predictions); umat=np.vstack(uniformity_predictions)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 48 | `candidate_df["p_defect_below_004"]=(dmat<.04).mean(0)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 49 | `candidate_df["uniformity_mean"]=umat.mean(0)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 50 | `safe=candidate_df.loc[candidate_df["p_defect_below_004"]>=.9]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 51 | `out=safe.sort_values("uniformity_mean",ascending=False).head(25)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 52 | `print(out.round(4))` | 결과를 콘솔에 출력합니다. |
| 53 | `out.to_csv(OUTPUT_DIR/"ex294_safe_optimization.csv",index=False,encoding="utf-8-sig")` | 추천 결과를 CSV로 저장합니다. |
