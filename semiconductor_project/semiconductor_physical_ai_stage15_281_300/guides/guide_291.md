...
```text
현재 최고 균일도 대비 Expected Improvement를 계산하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage15
python examples\ex291_expected_improvement.py
```
...
| 40 | `from scipy.stats import norm` | 필요한 라이브러리나 최적화 모델을 불러옵니다. |
| 41 | `predictions=[]` | 계산 결과나 설정값을 변수에 저장합니다. |
...
| 45 | `    predictions.append(m.predict(candidate_df[features]))` | 후보 공정 조건의 품질·불량률·시간을 예측합니다. |
| 46 | `mat=np.vstack(predictions)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 47 | `mean=mat.mean(0); std=mat.std(0)+1e-9` | 계산 결과나 설정값을 변수에 저장합니다. |
| 48 | `best=history_df["uniformity_percent"].max()` | 계산 결과나 설정값을 변수에 저장합니다. |
| 49 | `z=(mean-best)/std` | 계산 결과나 설정값을 변수에 저장합니다. |
| 50 | `ei=(mean-best)*norm.cdf(z)+std*norm.pdf(z)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 51 | `candidate_df["expected_improvement"]=ei` | 계산 결과나 설정값을 변수에 저장합니다. |
| 52 | `out=candidate_df.sort_values("expected_improvement",ascending=False).head(25)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 53 | `print(out.round(6))` | 결과를 콘솔에 출력합니다. |
| 54 | `out.to_csv(OUTPUT_DIR/"ex291_expected_improvement.csv",index=False,encoding="utf-8-sig")` | 추천 결과를 CSV로 저장합니다. |
