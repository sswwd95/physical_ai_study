# 실습 297 — sequential_batch_selection

## 1. 학습 목표
다음 실험 배치를 다양성 있게 선택합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
상위 후보 중 조건 간 거리를 고려해 다음 실험 12개를 선택하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage15
python examples\ex297_sequential_batch_selection.py
```
...
| 40 | `m=make_model()` | 계산 결과나 설정값을 변수에 저장합니다. |
| 41 | `m.fit(history_df[features],history_df["uniformity_percent"])` | 과거 공정 데이터로 대리모델을 학습합니다. |
| 42 | `candidate_df["score"]=m.predict(candidate_df[features])` | 후보 공정 조건의 품질·불량률·시간을 예측합니다. |
| 43 | `ranked=candidate_df.sort_values("score",ascending=False)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 44 | `selected=[]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 45 | `for _,row in ranked.iterrows():` | 여러 후보나 가중치 조합을 반복 계산합니다. |
| 46 | `    if len(selected)>=12:` | 계산 결과나 설정값을 변수에 저장합니다. |
| 47 | `        break` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 48 | `    if not selected:` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 49 | `        selected.append(row)` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 50 | `        continue` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 51 | `    distance=[` | 계산 결과나 설정값을 변수에 저장합니다. |
| 52 | `        abs(row["pressure_pa"]-x["pressure_pa"])` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 53 | `        + abs(row["rf_power_w"]-x["rf_power_w"])/20` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 54 | `        + abs(row["gas_flow_sccm"]-x["gas_flow_sccm"])/4` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 55 | `        + (row["recipe"]!=x["recipe"])*1.5` | 계산 결과나 설정값을 변수에 저장합니다. |
| 56 | `        for x in selected` | 여러 후보나 가중치 조합을 반복 계산합니다. |
| 57 | `    ]` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 58 | `    if min(distance)>=1.2:` | 계산 결과나 설정값을 변수에 저장합니다. |
| 59 | `        selected.append(row)` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 60 | `out=pd.DataFrame(selected)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 61 | `print(out.round(4))` | 결과를 콘솔에 출력합니다. |
| 62 | `out.to_csv(OUTPUT_DIR/"ex297_next_batch.csv",index=False,encoding="utf-8-sig")` | 추천 결과를 CSV로 저장합니다. |
