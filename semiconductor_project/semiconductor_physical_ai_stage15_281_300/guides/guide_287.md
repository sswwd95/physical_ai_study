...
```text
모든 후보 조건의 세 품질 결과를 예측해 저장하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage15
python examples\ex287_candidate_prediction.py
```
...
| 41 | `    model=make_model()` | 계산 결과나 설정값을 변수에 저장합니다. |
| 42 | `    model.fit(history_df[features],history_df[target])` | 과거 공정 데이터로 대리모델을 학습합니다. |
| 43 | `    candidate_df[f"predicted_{target}"]=model.predict(candidate_df[features])` | 후보 공정 조건의 품질·불량률·시간을 예측합니다. |
| 44 | `print(candidate_df.head().round(4))` | 결과를 콘솔에 출력합니다. |
| 45 | `candidate_df.to_csv(OUTPUT_DIR/"ex287_candidate_predictions.csv",index=False,encoding="utf-8-sig")` | 추천 결과를 CSV로 저장합니다. |
