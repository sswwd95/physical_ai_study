# 실습 344 — interlock_validation

## 1. 학습 목표
문·냉각·진공 인터록 위반을 검증합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
문·냉각·진공 인터록 위반과 강제정지 조건을 검증하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage18
python examples\ex344_interlock_validation.py
```
...
| 14 | `safe_df["interlock_violation"]=(` | 계산 결과나 안전 설정값을 변수에 저장합니다. |
| 15 | `    safe_df["door_closed"].eq(0)` | 이상 대응 또는 안전 의사결정 단계를 수행합니다. |
| 16 | `    \| safe_df["cooling_ok"].eq(0)` | 이상 대응 또는 안전 의사결정 단계를 수행합니다. |
| 17 | `    \| safe_df["vacuum_ok"].eq(0))` | 이상 대응 또는 안전 의사결정 단계를 수행합니다. |
| 18 | `safe_df["mandatory_stop"]=safe_df["interlock_violation"]` | 계산 결과나 안전 설정값을 변수에 저장합니다. |
| 19 | `print("인터록 위반:",int(safe_df["interlock_violation"].sum()))` | 결과를 콘솔에 출력합니다. |
| 20 | `print(safe_df.loc[safe_df["interlock_violation"],["timestamp","door_closed","cooling_ok","vacuum_ok"]].head())` | 결과를 콘솔에 출력합니다. |
