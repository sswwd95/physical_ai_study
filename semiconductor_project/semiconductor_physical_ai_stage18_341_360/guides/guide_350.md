# 실습 350 — action_policy

## 1. 학습 목표
관찰·재검사·감속·정지 행동을 선택합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
심각도에 따라 CONTINUE·MONITOR·REINSPECT·SLOWDOWN·STOP을 선택하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage18
python examples\ex350_action_policy.py
```
...
| 19 | `        safe_df["severity_level"]==1` | 계산 결과나 안전 설정값을 변수에 저장합니다. |
| 20 | `    ],` | 이상 대응 또는 안전 의사결정 단계를 수행합니다. |
| 21 | `    ["EMERGENCY_STOP","SLOWDOWN","REINSPECT","MONITOR"],` | 이상 대응 또는 안전 의사결정 단계를 수행합니다. |
| 22 | `    default="CONTINUE")` | 계산 결과나 안전 설정값을 변수에 저장합니다. |
| 23 | `print(safe_df["action"].value_counts())` | 결과를 콘솔에 출력합니다. |
