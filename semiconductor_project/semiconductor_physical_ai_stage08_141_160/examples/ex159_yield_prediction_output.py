...
result_df["actual_yield"] = y.iloc[test_index].to_numpy()
result_df["predicted_yield"] = prediction
result_df["absolute_error"] = np.abs(
    result_df["actual_yield"] - result_df["predicted_yield"]
)
result_df["predicted_low_yield"] = (
    result_df["predicted_yield"] < 92.0
)

result_df = result_df.sort_values("predicted_yield")

print(result_df.head(20).round(3))
result_df.to_csv(
    OUTPUT_DIR / "ex159_yield_predictions.csv",
    index=False,
    encoding="utf-8-sig",
)
