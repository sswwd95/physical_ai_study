# 실습 216 — cross_validation_faults

## 1. 학습 목표
층화 교차검증으로 성능 변동을 평가합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
StratifiedKFold 5분할 교차검증을 수행하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage11
python examples\ex216_cross_validation_faults.py
```
...
| 17 | `from sklearn.model_selection import StratifiedKFold,cross_validate` | 필요한 라이브러리나 분류 모델을 불러옵니다. |
...
| 24 | `m=Pipeline([("preprocess",pre),("classifier",RandomForestClassifier(n_estimators=250,class_weight="balanced",random_state=42,n_jobs=-1))])` | 전처리와 고장 분류 모델을 하나로 연결합니다. |
| 25 | `cv=StratifiedKFold(5,shuffle=True,random_state=42)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 26 | `s=cross_validate(m,X,y,cv=cv,scoring=["accuracy","f1_macro","f1_weighted"])` | 계산 결과나 설정값을 변수에 저장합니다. |
| 27 | `out=pd.DataFrame({"fold":range(1,6),"accuracy":s["test_accuracy"],"macro_f1":s["test_f1_macro"],"weighted_f1":s["test_f1_weighted"]})` | 계산 결과나 설정값을 변수에 저장합니다. |
| 28 | `print(out.round(4)); out.to_csv(OUTPUT_DIR/"ex216_cv_scores.csv",index=False,encoding="utf-8-sig")` | 결과를 CSV 파일로 저장합니다. |
