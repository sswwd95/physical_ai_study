# 실습 213 — confusion_matrix_analysis

## 1. 학습 목표
혼동행렬로 고장 유형별 오분류를 분석합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
라벨이 포함된 혼동행렬 CSV를 생성하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage11
python examples\ex213_confusion_matrix_analysis.py
```
...
| 17 | `from sklearn.metrics import confusion_matrix` | 필요한 라이브러리나 분류 모델을 불러옵니다. |
...
| 25 | `pre=ColumnTransformer([("num","passthrough",features[2:]),("cat",OneHotEncoder(handle_unknown="ignore"),features[:2])])` | 숫자형과 범주형 특징의 전처리를 묶습니다. |
| 26 | `m=Pipeline([("preprocess",pre),("classifier",RandomForestClassifier(n_estimators=350,class_weight="balanced",random_state=42,n_jobs=-1))])` | 전처리와 고장 분류 모델을 하나로 연결합니다. |
| 27 | `m.fit(Xtr,ytr); pred=m.predict(Xte); labels=m.classes_` | 학습 데이터로 모델을 학습합니다. |
| 28 | `mat=pd.DataFrame(confusion_matrix(yte,pred,labels=labels),index=[f"actual_{x}" for x in labels],columns=[f"pred_{x}" for x in labels])` | 실제 상태와 예측 상태의 조합을 표로 계산합니다. |
| 29 | `print(mat); mat.to_csv(OUTPUT_DIR/"ex213_confusion_matrix.csv",encoding="utf-8-sig")` | 실제 상태와 예측 상태의 조합을 표로 계산합니다. |
