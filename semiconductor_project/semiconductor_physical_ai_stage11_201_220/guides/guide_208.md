# 실습 208 — random_forest_faults

## 1. 학습 목표
Random Forest로 다중 고장을 분류합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
Random Forest로 다중 고장 유형을 분류하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage11
python examples\ex208_random_forest_faults.py
```
...
| 20 | `from sklearn.ensemble import RandomForestClassifier` | 필요한 라이브러리나 분류 모델을 불러옵니다. |
...
| 36 | `    ("classifier",RandomForestClassifier(n_estimators=400,max_depth=12,min_samples_leaf=4,class_weight="balanced",random_state=42,n_jobs=-1))])` | 계산 결과나 설정값을 변수에 저장합니다. |
