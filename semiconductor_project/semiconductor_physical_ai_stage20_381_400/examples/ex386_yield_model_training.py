...
X=data[features]; y=data["yield_percent"]
...
model=Pipeline([("preprocess",pre),("regressor",RandomForestRegressor(n_estimators=350,random_state=42,n_jobs=-1))])
model.fit(Xtr,ytr); pred=model.predict(Xte)
metrics={"mae":mean_absolute_error(yte,pred),"r2":r2_score(yte,pred)}
print(metrics)
