...
```text
범주형 전처리를 포함한 RandomForest 수율 예측 모델을 작성하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage20
python examples\ex386_yield_model_training.py
```
...
| 18 | `from sklearn.ensemble import RandomForestRegressor` | 필요한 라이브러리나 모델을 불러옵니다. |
...
| 25 | `X=data[features]; y=data["yield_percent"]` | 계산 결과나 설정값을 변수에 저장합니다. |
...
| 28 | `model=Pipeline([("preprocess",pre),("regressor",RandomForestRegressor(n_estimators=350,random_state=42,n_jobs=-1))])` | 계산 결과나 설정값을 변수에 저장합니다. |
| 29 | `model.fit(Xtr,ytr); pred=model.predict(Xte)` | 학습 데이터로 모델을 학습합니다. |
| 30 | `metrics={"mae":mean_absolute_error(yte,pred),"r2":r2_score(yte,pred)}` | 계산 결과나 설정값을 변수에 저장합니다. |
| 31 | `print(metrics)` | 결과를 콘솔에 출력합니다. |
