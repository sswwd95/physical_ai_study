...
```text
압력 수준 low·standard·high의 품질 평균을 비교하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage14
python examples\ex268_pressure_level_effect.py
```
...
| 14 | `codes,levels=pd.factorize(experiment_df["pressure_level"],sort=True)` | 계산 결과나 모형 요소를 변수에 저장합니다. |
| 15 | `with pm.Model(coords={"level":levels}) as model:` | PyMC 확률모형 범위를 시작합니다. |
| 16 | `    mu=pm.Normal("mu",95,3,dims="level"); sigma=pm.HalfNormal("sigma",2)` | 평균·효과·회귀계수에 정규 사전분포를 정의합니다. |
...
| 19 | `print(az.summary(idata,var_names=["mu"],hdi_prob=.94))` | 사후요약과 MCMC 진단값을 계산합니다. |
