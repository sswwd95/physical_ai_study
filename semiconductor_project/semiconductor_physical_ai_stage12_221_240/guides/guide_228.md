...
python examples\ex228_failure_classifier_random_forest.py
```
...
| 22 | `model=RandomForestClassifier(n_estimators=350,class_weight="balanced",random_state=42,n_jobs=-1)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 23 | `model.fit(X.iloc[tr],y.iloc[tr])` | 학습 데이터로 모델을 학습합니다. |
| 24 | `pred=model.predict(X.iloc[te])` | 고장 임박 여부 또는 잔여수명을 예측합니다. |
| 25 | `print(classification_report(y.iloc[te],pred,zero_division=0))` | 결과를 콘솔에 출력합니다. |
