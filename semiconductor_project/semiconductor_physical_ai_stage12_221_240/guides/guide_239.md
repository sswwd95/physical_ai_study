...
python examples\ex239_maintenance_schedule_output.py
```
...
| 17 | `latest["recommended_action"]=np.select(` | 계산 결과나 설정값을 변수에 저장합니다. |
| 18 | `    [latest["rul_cycles"]<=10,latest["rul_cycles"]<=20,latest["rul_cycles"]<=40],` | 계산 결과나 설정값을 변수에 저장합니다. |
| 19 | `    ["immediate_maintenance","schedule_within_week","inspection_required"],` | 예지보전 분석 단계를 수행합니다. |
| 20 | `    default="continue_monitoring")` | 계산 결과나 설정값을 변수에 저장합니다. |
| 21 | `latest["recommended_due_cycle"]=latest["cycle"]+latest["rul_cycles"].clip(lower=0)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 22 | `out=latest[["equipment_id","cycle","health_index","rul_cycles","recommended_action","recommended_due_cycle"]].sort_values("rul_cycles")` | 계산 결과나 설정값을 변수에 저장합니다. |
| 23 | `print(out)` | 결과를 콘솔에 출력합니다. |
| 24 | `out.to_csv(OUTPUT_DIR/"ex239_maintenance_schedule.csv",index=False,encoding="utf-8-sig")` | 결과를 CSV로 저장합니다. |
