...
python examples\ex050_run_rule_one_side.py
```

## 4. 예상 결과
중심선 한쪽에 7개 연속 위치한 구간의 끝 행이 탐지됩니다.
...
| 15 | `center_line = sensor_df["chamber_temp_c"].mean()` | 데이터의 평균을 계산합니다. |
| 16 | `sensor_df["side"] = np.where(` | 오른쪽 계산 결과를 왼쪽 변수에 저장합니다. |
| 17 | `    sensor_df["chamber_temp_c"] >= center_line,` | 오른쪽 계산 결과를 왼쪽 변수에 저장합니다. |
| 18 | `    1,` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 19 | `    -1,` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 20 | `)` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 21 | `` | 코드 구역을 보기 좋게 나누는 빈 줄입니다. |
| 22 | `window_size = 7` | 오른쪽 계산 결과를 왼쪽 변수에 저장합니다. |
| 23 | `rolling_sum = (` | 오른쪽 계산 결과를 왼쪽 변수에 저장합니다. |
| 24 | `    sensor_df["side"]` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 25 | `    .rolling(window=window_size)` | 지정한 구간의 이동통계를 계산합니다. |
| 26 | `    .sum()` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 27 | `)` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 28 | `` | 코드 구역을 보기 좋게 나누는 빈 줄입니다. |
| 29 | `sensor_df["run_rule_violation"] = (` | 오른쪽 계산 결과를 왼쪽 변수에 저장합니다. |
| 30 | `    rolling_sum.abs() == window_size` | 오른쪽 계산 결과를 왼쪽 변수에 저장합니다. |
| 31 | `)` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 32 | `` | 코드 구역을 보기 좋게 나누는 빈 줄입니다. |
| 33 | `violation_df = sensor_df.loc[` | 오른쪽 계산 결과를 왼쪽 변수에 저장합니다. |
| 34 | `    sensor_df["run_rule_violation"],` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 35 | `    ["timestamp", "chamber_temp_c", "side"],` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 36 | `]` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 37 | `` | 코드 구역을 보기 좋게 나누는 빈 줄입니다. |
| 38 | `print("런 규칙 위반 행 수:", len(violation_df))` | 실행 결과를 콘솔에 출력합니다. |
| 39 | `violation_df.to_csv(` | 계산 결과를 CSV 파일로 저장합니다. |
| 40 | `    OUTPUT_DIR / "ex050_run_rule_violations.csv",` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 41 | `    index=False,` | 오른쪽 계산 결과를 왼쪽 변수에 저장합니다. |
| 42 | `    encoding="utf-8-sig",` | 오른쪽 계산 결과를 왼쪽 변수에 저장합니다. |
