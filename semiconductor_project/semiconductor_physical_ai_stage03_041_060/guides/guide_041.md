...
## 1. 학습 목표
주요 공정 변수의 평균, 표준편차, 최소, 최대, 중앙값을 한 번에 요약합니다.
...
python examples\ex041_process_variable_summary.py
```

## 4. 예상 결과
5개 공정 변수의 핵심 통계량이 표로 출력되고 CSV로 저장됩니다.
...
| 20 | `    "vibration_g",` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 21 | `]` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 22 | `` | 코드 구역을 보기 좋게 나누는 빈 줄입니다. |
| 23 | `summary = sensor_df[sensor_columns].agg(` | 오른쪽 계산 결과를 왼쪽 변수에 저장합니다. |
| 24 | `    ["mean", "std", "min", "median", "max"]` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 25 | `).T` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 26 | `` | 코드 구역을 보기 좋게 나누는 빈 줄입니다. |
| 27 | `print(summary.round(3))` | 실행 결과를 콘솔에 출력합니다. |
| 28 | `summary.to_csv(` | 계산 결과를 CSV 파일로 저장합니다. |
| 29 | `    OUTPUT_DIR / "ex041_process_variable_summary.csv",` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 30 | `    encoding="utf-8-sig",` | 오른쪽 계산 결과를 왼쪽 변수에 저장합니다. |
