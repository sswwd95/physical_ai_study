...
## 1. 학습 목표
Decision Tree 깊이에 따른 과소적합·과적합 변화를 비교합니다.
...
python examples\ex109_tree_depth_comparison.py
```

## 4. 예상 결과
트리 깊이에 따른 학습·평가 F1 차이로 과적합 여부를 확인합니다.
...
| 16 | `from sklearn.metrics import f1_score` | 필요한 라이브러리나 모델을 불러옵니다. |
...
| 51 | `for depth in [2, 3, 5, 8, None]:` | 여러 설정이나 모델을 같은 방식으로 반복 평가합니다. |
...
| 56 | `            DecisionTreeClassifier(` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 57 | `                max_depth=depth,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 58 | `                min_samples_leaf=5,` | 계산 결과나 설정값을 변수에 저장합니다. |
...
| 65 | `    rows.append({` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 66 | `        "max_depth": str(depth),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 67 | `        "train_f1": f1_score(` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 68 | `            y_train,` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 69 | `            model.predict(x_train),` | 학습된 모델로 정상·불량 클래스를 예측합니다. |
| 70 | `            zero_division=0,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 71 | `        ),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 72 | `        "test_f1": f1_score(` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 73 | `            y_test,` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 74 | `            model.predict(x_test),` | 학습된 모델로 정상·불량 클래스를 예측합니다. |
| 75 | `            zero_division=0,` | 계산 결과나 설정값을 변수에 저장합니다. |
...
| 80 | `print(result_df.round(4))` | 실행 결과를 콘솔에 출력합니다. |
| 81 | `result_df.to_csv(` | 결과를 CSV 파일로 저장합니다. |
| 82 | `    OUTPUT_DIR / "ex109_tree_depth_comparison.csv",` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
