...
## 1. 학습 목표
ROC-AUC로 여러 모델의 전체 임계값 분류 능력을 비교합니다.
...
python examples\ex116_roc_auc_comparison.py
```

## 4. 예상 결과
세 모델의 ROC-AUC가 비교되어 전체 임계값 분류 능력을 확인합니다.
...
| 18 | `from sklearn.metrics import roc_auc_score` | 필요한 라이브러리나 모델을 불러옵니다. |
...
| 52 | `tree_preprocessor = ColumnTransformer([` | 숫자형과 범주형 컬럼의 전처리를 하나로 묶습니다. |
| 53 | `    ("num", "passthrough", numeric_features),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 54 | `    ("cat", OneHotEncoder(handle_unknown="ignore"), categorical_features),` | 계산 결과나 설정값을 변수에 저장합니다. |
| 55 | `])` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 56 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 57 | `models = {` | 계산 결과나 설정값을 변수에 저장합니다. |
| 58 | `    "LogisticRegression": Pipeline([` | 전처리와 모델 학습 과정을 하나의 파이프라인으로 연결합니다. |
| 59 | `        ("preprocess", linear_preprocessor),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 60 | `        (` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 61 | `            "classifier",` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 62 | `            LogisticRegression(` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 63 | `                max_iter=1000,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 64 | `                class_weight="balanced",` | 계산 결과나 설정값을 변수에 저장합니다. |
| 65 | `                random_state=42,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 66 | `            ),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 67 | `        ),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 68 | `    ]),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 69 | `    "DecisionTree": Pipeline([` | 전처리와 모델 학습 과정을 하나의 파이프라인으로 연결합니다. |
| 70 | `        ("preprocess", tree_preprocessor),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 71 | `        (` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 72 | `            "classifier",` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 73 | `            DecisionTreeClassifier(` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 74 | `                max_depth=5,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 75 | `                class_weight="balanced",` | 계산 결과나 설정값을 변수에 저장합니다. |
| 76 | `                random_state=42,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 77 | `            ),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 78 | `        ),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 79 | `    ]),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 80 | `    "RandomForest": Pipeline([` | 전처리와 모델 학습 과정을 하나의 파이프라인으로 연결합니다. |
| 81 | `        ("preprocess", tree_preprocessor),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 82 | `        (` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 83 | `            "classifier",` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 84 | `            RandomForestClassifier(` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 85 | `                n_estimators=300,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 86 | `                class_weight="balanced",` | 계산 결과나 설정값을 변수에 저장합니다. |
| 87 | `                random_state=42,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 88 | `                n_jobs=-1,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 89 | `            ),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 90 | `        ),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 91 | `    ]),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 92 | `}` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 93 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 94 | `rows = []` | 계산 결과나 설정값을 변수에 저장합니다. |
| 95 | `for name, model in models.items():` | 여러 설정이나 모델을 같은 방식으로 반복 평가합니다. |
| 96 | `    model.fit(x_train, y_train)` | 학습 데이터로 전처리 기준과 분류 모델을 학습합니다. |
| 97 | `    probability = model.predict_proba(x_test)[:, 1]` | 각 행이 불량일 확률을 계산합니다. |
| 98 | `    rows.append({` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 99 | `        "model": name,` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 100 | `        "roc_auc": roc_auc_score(y_test, probability),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 101 | `    })` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 102 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 103 | `result_df = pd.DataFrame(rows).sort_values("roc_auc", ascending=False)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 104 | `print(result_df.round(4))` | 실행 결과를 콘솔에 출력합니다. |
| 105 | `result_df.to_csv(` | 결과를 CSV 파일로 저장합니다. |
| 106 | `    OUTPUT_DIR / "ex116_roc_auc_comparison.csv",` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
