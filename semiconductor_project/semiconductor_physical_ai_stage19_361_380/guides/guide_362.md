...
python examples\ex362_config_loader.py
```
...
| 20 | `required_keys=["project_name","input_file","output_dir","log_level","yield_warning_threshold","batch_size"]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 21 | `missing=[k for k in required_keys if k not in config]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 22 | `if missing:` | 시스템 통합 또는 운영 자동화 단계를 수행합니다. |
| 23 | `    raise KeyError(f"누락 설정: {missing}")` | 시스템 통합 또는 운영 자동화 단계를 수행합니다. |
| 24 | `if config["batch_size"]<=0:` | 계산 결과나 설정값을 변수에 저장합니다. |
| 25 | `    raise ValueError("batch_size는 1 이상이어야 합니다.")` | 시스템 통합 또는 운영 자동화 단계를 수행합니다. |
| 26 | `print(json.dumps(config,ensure_ascii=False,indent=2))` | 결과를 콘솔에 출력합니다. |
