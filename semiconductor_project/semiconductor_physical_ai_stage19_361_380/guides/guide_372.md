...
python examples\ex372_data_drift_check.py
```
...
| 19 | `split=int(len(ops_df)*.7)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 20 | `train=ops_df.iloc[:split]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 21 | `current=ops_df.iloc[split:]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 22 | `features=["temperature_c","pressure_pa","vibration_rms_g","particle_count","cycle_time_min","yield_percent"]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 23 | `rows=[]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 24 | `for col in features:` | 여러 파일·배치·장비를 반복 처리합니다. |
| 25 | `    mean_diff=abs(current[col].mean()-train[col].mean())/(train[col].std()+1e-9)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 26 | `    std_ratio=current[col].std()/(train[col].std()+1e-9)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 27 | `    rows.append({"feature":col,"standardized_mean_diff":mean_diff,"std_ratio":std_ratio,"drift_alert":(mean_diff>.5)or(std_ratio<.7)or(std_ratio>1.3)})` | 시스템 통합 또는 운영 자동화 단계를 수행합니다. |
| 28 | `out=pd.DataFrame(rows)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 29 | `print(out.round(4))` | 결과를 콘솔에 출력합니다. |
| 30 | `out.to_csv(OUTPUT_DIR/"ex372_data_drift.csv",index=False,encoding="utf-8-sig")` | 결과를 CSV로 저장합니다. |
