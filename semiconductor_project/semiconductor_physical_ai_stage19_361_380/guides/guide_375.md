...
python examples\ex375_daily_kpi_aggregation.py
```
...
| 24 | `    running_ratio=("process_status",lambda s:(s=="RUNNING").mean())` | 계산 결과나 설정값을 변수에 저장합니다. |
| 25 | `)` | 시스템 통합 또는 운영 자동화 단계를 수행합니다. |
| 26 | `print(daily.round(4))` | 결과를 콘솔에 출력합니다. |
| 27 | `daily.to_csv(OUTPUT_DIR/"ex375_daily_kpi.csv",encoding="utf-8-sig")` | 결과를 CSV로 저장합니다. |
