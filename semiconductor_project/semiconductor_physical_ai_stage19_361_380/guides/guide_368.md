...
python examples\ex368_model_training_pipeline.py
```
...
| 18 | `from sklearn.ensemble import RandomForestClassifier` | 필요한 라이브러리를 불러옵니다. |
| 19 | `from sklearn.metrics import classification_report` | 필요한 라이브러리를 불러옵니다. |
| 20 | `from sklearn.model_selection import train_test_split` | 필요한 라이브러리를 불러옵니다. |
...
| 23 | `X=ops_df[features]; y=ops_df["fault_flag"]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 24 | `Xtr,Xte,ytr,yte=train_test_split(X,y,test_size=.25,random_state=42,stratify=y)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 25 | `model=RandomForestClassifier(n_estimators=300,class_weight="balanced",random_state=42,n_jobs=-1)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 26 | `model.fit(Xtr,ytr)` | 운영 데이터로 모델을 학습합니다. |
| 27 | `pred=model.predict(Xte)` | 모델 예측값을 계산합니다. |
| 28 | `print(classification_report(yte,pred,zero_division=0))` | 결과를 콘솔에 출력합니다. |
