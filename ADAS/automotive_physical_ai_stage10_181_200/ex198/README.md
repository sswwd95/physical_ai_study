...
| 6 | `    mu = pm.Normal("mu", 0.08, 0.08)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 7 | `    sigma = pm.HalfNormal("sigma", 0.08)` | 양수인 표준편차의 사전분포를 정의합니다. |
| 8 | `    pm.Normal("obs", mu=mu, sigma=sigma, observed=y)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 9 | `idata = sample_model(model)` | MCMC 샘플링으로 사후분포를 생성합니다. |
| 10 | `summary = az.summary(idata, var_names=["mu","sigma"], round_to=6)` | 데이터, 모델 모수, 통계량 또는 판정값을 계산합니다. |
| 11 | `path = output_path("ex198_diagnostics.csv")` | 데이터, 모델 모수, 통계량 또는 판정값을 계산합니다. |
| 12 | `summary.to_csv(path, encoding="utf-8-sig")` | 분석 결과를 outputs 폴더에 저장합니다. |
| 13 | `print(summary[["r_hat","ess_bulk","ess_tail"]])` | 핵심 추정값·진단값·저장 경로를 출력합니다. |
