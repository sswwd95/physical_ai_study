...
| 5 | `    bias = pm.Normal("bias", mu=0.0, sigma=0.20)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 6 | `    sigma = pm.HalfNormal("sigma", sigma=0.20)` | 양수인 표준편차의 사전분포를 정의합니다. |
| 7 | `    pm.Normal("obs", mu=bias, sigma=sigma, observed=y)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 8 | `idata = sample_model(model)` | MCMC 샘플링으로 사후분포를 생성합니다. |
| 9 | `summary, path = save_summary(idata, ["bias","sigma"], "ex189_distance_error.csv")` | 데이터, 모델 모수, 통계량 또는 판정값을 계산합니다. |
