...
| 6 | `n_surface = int(df["surface_id"].nunique())` | 데이터, 모델 모수, 통계량 또는 판정값을 계산합니다. |
| 7 | `with pm.Model() as model:` | 확률 모델 정의를 시작합니다. |
| 8 | `    surface_mu = pm.Normal("surface_mu", mu=0.08, sigma=0.08, shape=n_surface)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 9 | `    sigma = pm.HalfNormal("sigma", sigma=0.08)` | 양수인 표준편차의 사전분포를 정의합니다. |
| 10 | `    pm.Normal("obs", mu=surface_mu[sid], sigma=sigma, observed=y)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 11 | `idata = sample_model(model)` | MCMC 샘플링으로 사후분포를 생성합니다. |
| 12 | `summary, path = save_summary(idata, ["surface_mu","sigma"], "ex187_surface_compare.csv")` | 데이터, 모델 모수, 통계량 또는 판정값을 계산합니다. |
