...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `from common.safety_utils import load_data` | 안전거리·충돌 위험 분석에 필요한 라이브러리를 불러옵니다. |
| 2 | `df=load_data()` | 합성 충돌 위험 로그를 읽습니다. |
| 3 | `df["avoidance_direction"]=df["obstacle_angle_deg"].map(lambda a:"RIGHT" if a>=0 else "LEFT")` | 거리, TTC, 감속, 회피 또는 평가값을 계산합니다. |
| 4 | `print(df.groupby("avoidance_direction")["distance_m"].agg(["count","mean"]))` | 위험 수치, 성능 또는 저장 경로를 출력합니다. |
