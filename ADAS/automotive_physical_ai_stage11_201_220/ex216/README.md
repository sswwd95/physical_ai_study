# 예제 216 — Isolation Forest 점수 분석

## 학습 목표
자동차 센서 로그에서 **Isolation Forest 점수 분석** 절차를 수행하고 ROS2 실시간 진단으로 연결하는 기초를 익힙니다.

## 실행
```bat
cd /d C:\work\automotive_physical_ai_stage11_201_220
conda activate auto_physical_ai
python ex216\main.py
```
...
| 6 | `model.fit(features)` | 현재 주행 이상 분석 절차를 실행합니다. |
| 7 | `df["anomaly_score"] = -model.score_samples(features)` | 주행 상태, 이상 조건, 위험 점수 또는 평가값을 계산합니다. |
| 8 | `path = output_path("ex216_isolation_scores.csv")` | 주행 상태, 이상 조건, 위험 점수 또는 평가값을 계산합니다. |
| 9 | `df.sort_values("anomaly_score",ascending=False).head(100).to_csv(path,index=False,encoding="utf-8-sig")` | 분석 결과를 outputs 폴더에 저장합니다. |
| 10 | `print(df[["time_s","anomaly_score","event_label"]].sort_values("anomaly_score",ascending=False).head())` | 탐지 개수, 임계값, 평가 지표 또는 저장 경로를 출력합니다. |
