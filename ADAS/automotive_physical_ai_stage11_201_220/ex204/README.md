...
## 학습 목표
자동차 센서 로그에서 **급감속 탐지** 절차를 수행하고 ROS2 실시간 진단으로 연결하는 기초를 익힙니다.

## 실행
```bat
cd /d C:\work\automotive_physical_ai_stage11_201_220
conda activate auto_physical_ai
python ex204\main.py
```
...
| 3 | `events = df[df["accel_mps2"] < -2.0].copy()` | 주행 상태, 이상 조건, 위험 점수 또는 평가값을 계산합니다. |
| 4 | `path = output_path("ex204_hard_braking.csv")` | 주행 상태, 이상 조건, 위험 점수 또는 평가값을 계산합니다. |
