...
| 4 | `path = out("ex083_common_10hz_grid.csv")` | 동기화, 보간, 융합 또는 통계 계산에 필요한 값을 만듭니다. |
| 5 | `grid.to_csv(path, index=False)` | 분석 결과를 outputs 폴더에 저장합니다. |
| 6 | `print(grid.head())` | 핵심 결과를 콘솔에 출력합니다. |
| 7 | `print("rows:", len(grid), "saved:", path)` | 핵심 결과를 콘솔에 출력합니다. |
