...
| 2 | `from stable_baselines3.common.callbacks import CheckpointCallback` | Stable-Baselines3·Gymnasium·분석 모듈을 불러옵니다. |
| 3 | `from envs.simple_car_env import SimpleCarEnv` | Stable-Baselines3·Gymnasium·분석 모듈을 불러옵니다. |
| 4 | `from common.sb3_utils import model_path` | Stable-Baselines3·Gymnasium·분석 모듈을 불러옵니다. |
| 5 | `env=SimpleCarEnv(max_steps=100)` | 모델, 환경, 행동, 보상 또는 통계값을 계산합니다. |
| 6 | `callback=CheckpointCallback(save_freq=300,save_path=str(model_path("ex391_checkpoints")),name_prefix="ppo_car")` | 모델, 환경, 행동, 보상 또는 통계값을 계산합니다. |
| 7 | `model=PPO("MlpPolicy",env,verbose=0,seed=42,n_steps=128,batch_size=64)` | PPO 정책과 학습 하이퍼파라미터를 설정합니다. |
| 8 | `model.learn(total_timesteps=900,callback=callback)` | 지정한 timesteps 동안 PPO 정책을 학습합니다. |
| 9 | `print("checkpoint complete")` | 학습 상태, 평가 결과 또는 저장 경로를 출력합니다. |
