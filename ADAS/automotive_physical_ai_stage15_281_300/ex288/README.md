...
| 3 | `df=load_lifetime(); y=df[(df["component"]=="bearing")&(df["failure_event"]==1)]["observed_time_h"].to_numpy()` | 수명 또는 RUL 데이터를 읽습니다. |
| 4 | `with pm.Model() as model:` | 확률 모델 정의를 시작합니다. |
| 5 | `    alpha=pm.HalfNormal("alpha",5)` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
| 6 | `    beta=pm.HalfNormal("beta",1800)` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
| 7 | `    pm.Weibull("obs",alpha=alpha,beta=beta,observed=y)` | 부품 수명에 Weibull 분포를 적용합니다. |
| 8 | `idata=sample_model(model)` | MCMC 샘플링으로 사후분포를 생성합니다. |
| 9 | `s,pth=save_summary(idata,["alpha","beta"],"ex288_bearing_weibull.csv")` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
| 10 | `print(s); print(pth)` | 추정값, 진단값 또는 저장 경로를 출력합니다. |
