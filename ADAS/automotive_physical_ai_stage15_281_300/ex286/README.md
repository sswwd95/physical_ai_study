...
| 3 | `df=load_lifetime()` | 수명 또는 RUL 데이터를 읽습니다. |
| 4 | `y=df.loc[df["failure_event"]==1,"observed_time_h"].to_numpy()` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
| 5 | `with pm.Model() as model:` | 확률 모델 정의를 시작합니다. |
| 6 | `    alpha=pm.HalfNormal("shape_alpha",5)` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
| 7 | `    beta=pm.HalfNormal("scale_beta",2000)` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
| 8 | `    pm.Weibull("obs",alpha=alpha,beta=beta,observed=y)` | 부품 수명에 Weibull 분포를 적용합니다. |
| 9 | `idata=sample_model(model)` | MCMC 샘플링으로 사후분포를 생성합니다. |
| 10 | `s,pth=save_summary(idata,["shape_alpha","scale_beta"],"ex286_weibull_summary.csv")` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
| 11 | `print(s); print(pth)` | 추정값, 진단값 또는 저장 경로를 출력합니다. |
