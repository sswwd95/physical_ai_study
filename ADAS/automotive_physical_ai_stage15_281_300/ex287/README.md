...
| 3 | `df=load_lifetime(); y=df[(df["component"]=="motor")&(df["failure_event"]==1)]["observed_time_h"].to_numpy()` | 수명 또는 RUL 데이터를 읽습니다. |
| 4 | `with pm.Model() as model:` | 확률 모델 정의를 시작합니다. |
| 5 | `    alpha=pm.HalfNormal("alpha",5)` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
| 6 | `    beta=pm.HalfNormal("beta",2000)` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
...
| 9 | `s,pth=save_summary(idata,["alpha","beta"],"ex287_motor_weibull.csv")` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
