...
| 3 | `y=load_lifetime()["failure_event"].to_numpy()` | 수명 또는 RUL 데이터를 읽습니다. |
| 4 | `with pm.Model() as model:` | 확률 모델 정의를 시작합니다. |
| 5 | `    p=pm.Beta("failure_probability",2,18)` | 0~1 고장확률에 베타 사전분포를 설정합니다. |
| 6 | `    pm.Bernoulli("obs",p=p,observed=y)` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
| 7 | `idata=sample_model(model)` | MCMC 샘플링으로 사후분포를 생성합니다. |
| 8 | `s,pth=save_summary(idata,["failure_probability"],"ex284_failure_probability.csv")` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
