...
## 학습 목표
자동차와 TurtleBot3에 공통으로 사용되는 IMU·엔코더의 **가속도 측정 오차 계산** 원리를 익힙니다.

## 실행 방법
```bat
cd /d C:\work\automotive_physical_ai_stage8_141_160
conda activate auto_physical_ai
python ex142\main.py
```
...
| 3 | `error = df["imu_ax_mps2"] - df["true_accel_mps2"]` | 센서 오차, 보정값, 적분 결과 또는 탐지 조건을 계산합니다. |
...
| 6 | `print("RMSE:", round(rmse(df["imu_ax_mps2"], df["true_accel_mps2"]), 6))` | 기준값과 센서값 사이의 평균제곱근 오차를 계산합니다. |
