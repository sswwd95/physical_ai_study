...
## 학습 목표
자동차와 TurtleBot3에 공통으로 사용되는 IMU·엔코더의 **자이로 드리프트 시각화** 원리를 익힙니다.

## 실행 방법
```bat
cd /d C:\work\automotive_physical_ai_stage8_141_160
conda activate auto_physical_ai
python ex148\main.py
```
...
| 6 | `error = df["imu_gyroz_rps"] - df["true_yaw_rate_rps"]` | 센서 오차, 보정값, 적분 결과 또는 탐지 조건을 계산합니다. |
| 7 | `fig, ax = plt.subplots(figsize=(10,4))` | 센서 오차, 보정값, 적분 결과 또는 탐지 조건을 계산합니다. |
| 8 | `ax.plot(df["time_s"], error)` | 현재 센서 분석 절차를 실행합니다. |
| 9 | `ax.set_title("Gyroscope Error and Drift")` | 현재 센서 분석 절차를 실행합니다. |
| 10 | `ax.set_xlabel("Time (s)")` | 현재 센서 분석 절차를 실행합니다. |
| 11 | `ax.set_ylabel("Error (rad/s)")` | 현재 센서 분석 절차를 실행합니다. |
| 12 | `ax.grid(True)` | 현재 센서 분석 절차를 실행합니다. |
| 13 | `path = output_path("ex148_gyro_drift.png")` | 센서 오차, 보정값, 적분 결과 또는 탐지 조건을 계산합니다. |
