...
## 학습 목표
자동차와 TurtleBot3에 공통으로 사용되는 IMU·엔코더의 **엔코더 틱 변화량 분석** 원리를 익힙니다.

## 실행 방법
```bat
cd /d C:\work\automotive_physical_ai_stage8_141_160
conda activate auto_physical_ai
python ex151\main.py
```
...
| 3 | `tick_diff = df["encoder_ticks"].diff().fillna(0)` | 센서 오차, 보정값, 적분 결과 또는 탐지 조건을 계산합니다. |
| 4 | `print(tick_diff.describe())` | 바이어스, 오차, 품질 지표 또는 저장 경로를 출력합니다. |
| 5 | `print("zero tick samples:", int((tick_diff == 0).sum()))` | 바이어스, 오차, 품질 지표 또는 저장 경로를 출력합니다. |
