...
## 학습 목표
자동차 센서 로그에서 **급감속 구간 탐지** 작업을 수행하고, 결과를 ROS2 주행 데이터 분석에 연결합니다.
...
| 4 | `events = df[df["accel_mps2"] <= -1.8][` | 분석에 필요한 변수·파생 열·조건·집계 결과를 계산합니다. |
| 5 | `    ["timestamp", "time_s", "speed_mps", "accel_mps2", "brake_pct"]` | 현재 분석 절차를 실행합니다. |
| 6 | `].copy()` | 현재 분석 절차를 실행합니다. |
| 7 | `path = output_path("ex070_hard_braking.csv")` | 결과가 공통 outputs 폴더에 저장되도록 경로를 만듭니다. |
| 8 | `events.to_csv(path, index=False, encoding="utf-8-sig")` | 분석 결과를 파일로 저장합니다. |
| 9 | `print(f"hard braking samples: {len(events)}")` | 실행 결과와 핵심 요약값을 콘솔에 출력합니다. |
