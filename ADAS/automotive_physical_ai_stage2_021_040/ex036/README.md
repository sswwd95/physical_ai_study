...
- **04행** `summary = df.groupby("status")[["speed_mps", "front_distance_m", "motor_temp_c"]].agg(["count", "mean", "std"])` — 센서 값, 조건식 또는 계산 결과를 변수에 저장합니다.
- **05행** `print(summary.round(3))` — 계산 결과나 상태를 화면에 출력해 확인합니다.
