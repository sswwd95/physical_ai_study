...
- **04행** `dt = df["time_s"].diff()` — 센서 값, 조건식 또는 계산 결과를 변수에 저장합니다.
- **05행** `df["estimated_accel"] = df["speed_mps"].diff() / dt` — 센서 값, 조건식 또는 계산 결과를 변수에 저장합니다.
- **06행** `candidates = df.loc[df["estimated_accel"].abs() > 2.5, ["time_s", "speed_mps", "estimated_accel"]]` — 센서 값, 조건식 또는 계산 결과를 변수에 저장합니다.
- **07행** `print(candidates.head(20).round(3))` — 계산 결과나 상태를 화면에 출력해 확인합니다.
- **08행** `print("급변 후보 수:", len(candidates))` — 계산 결과나 상태를 화면에 출력해 확인합니다.
