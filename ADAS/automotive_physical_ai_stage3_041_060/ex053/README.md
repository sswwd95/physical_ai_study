# 예제 053 — Robust Z-score 이상값 탐지

## 학습 목표
- 자동차 센서 로그에서 Robust Z-score 이상값 탐지 절차를 수행합니다.
...
| 10 | `from common.preprocessing import robust_zscore` | 실습에 필요한 모듈이나 경로 도구를 불러옵니다. |
| 11 | `` | 코드 구간을 구분해 가독성을 높입니다. |
| 12 | `# 중앙값과 MAD를 이용해 조향각의 강건한 점수를 계산한다.` | 중앙값과 MAD를 이용해 조향각의 강건한 점수를 계산한다. |
| 13 | `df = load_sensor_log()` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 14 | `score = robust_zscore(df["steering_deg"])` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 15 | `mask = score.abs() > 3.5` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 16 | `print(df.loc[mask, ["timestamp", "steering_deg"]].assign(score=score[mask].round(2)))` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
