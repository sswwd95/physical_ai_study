...
## 학습 목표
- 자동차 센서 로그에서 시간 기반 보간 절차를 수행합니다.
...
| 11 | `# 타임스탬프를 인덱스로 사용한다.` | 타임스탬프를 인덱스로 사용한다. |
| 12 | `df = load_sensor_log().sort_values("timestamp").drop_duplicates("timestamp")` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 13 | `df = df.set_index("timestamp")` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 14 | `# 실제 시간 간격을 반영하여 보간한다.` | 실제 시간 간격을 반영하여 보간한다. |
| 15 | `df["distance_time_interp"] = df["front_distance_m"].interpolate(method="time", limit=5)` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 16 | `print(df[["front_distance_m", "distance_time_interp"]].iloc[85:100])` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
