...
## 학습 목표
- 자동차 센서 로그에서 리샘플링으로 주기 통일 절차를 수행합니다.
...
| 11 | `# 중복 시각을 제거하고 타임스탬프를 인덱스로 설정한다.` | 중복 시각을 제거하고 타임스탬프를 인덱스로 설정한다. |
| 12 | `df = load_sensor_log().sort_values("timestamp").drop_duplicates("timestamp").set_index("timestamp")` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 13 | `# 200ms 간격으로 평균 리샘플링한다.` | 200ms 간격으로 평균 리샘플링한다. |
| 14 | `numeric = df.select_dtypes(include="number")` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 15 | `resampled = numeric.resample("200ms").mean().interpolate("time")` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 16 | `print(resampled.head(10))` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
