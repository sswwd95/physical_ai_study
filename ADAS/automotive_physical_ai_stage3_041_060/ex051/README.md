# 예제 051 — Z-score 이상값 탐지

## 학습 목표
- 자동차 센서 로그에서 Z-score 이상값 탐지 절차를 수행합니다.
...
| 11 | `# 속도 결측값을 제외하고 평균과 표준편차를 계산한다.` | 속도 결측값을 제외하고 평균과 표준편차를 계산한다. |
| 12 | `df = load_sensor_log()` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 13 | `series = df["vehicle_speed_kmh"]` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 14 | `zscore = (series - series.mean()) / series.std(ddof=0)` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 15 | `# 절댓값 3 이상을 이상값으로 본다.` | 절댓값 3 이상을 이상값으로 본다. |
| 16 | `outliers = df[zscore.abs() >= 3]` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 17 | `print(outliers[["timestamp", "vehicle_speed_kmh"]])` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
