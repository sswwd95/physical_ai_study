# 예제 042 — 시간순 정렬과 중복 제거

## 학습 목표
- 자동차 센서 로그에서 시간순 정렬과 중복 제거 절차를 수행합니다.
...
| 11 | `# 원본 로그를 읽는다.` | 원본 로그를 읽는다. |
| 12 | `df = load_sensor_log()` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 13 | `# 타임스탬프 기준으로 정렬한다.` | 타임스탬프 기준으로 정렬한다. |
| 14 | `df = df.sort_values("timestamp")` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 15 | `# 같은 시각의 중복 행은 마지막 행만 남긴다.` | 같은 시각의 중복 행은 마지막 행만 남긴다. |
| 16 | `df = df.drop_duplicates(subset="timestamp", keep="last")` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 17 | `# 인덱스를 0부터 다시 부여한다.` | 인덱스를 0부터 다시 부여한다. |
| 18 | `df = df.reset_index(drop=True)` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 19 | `print(df.head())` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
| 20 | `print("monotonic:", df["timestamp"].is_monotonic_increasing)` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
