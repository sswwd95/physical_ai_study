# 예제 050 — 클리핑으로 센서 범위 제한

## 학습 목표
- 자동차 센서 로그에서 클리핑으로 센서 범위 제한 절차를 수행합니다.
...
| 11 | `# 속도 센서의 비정상 범위를 제한한다.` | 속도 센서의 비정상 범위를 제한한다. |
| 12 | `df = load_sensor_log()` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 13 | `df["speed_clipped"] = df["vehicle_speed_kmh"].clip(lower=0, upper=130)` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 14 | `# 변경된 행만 확인한다.` | 변경된 행만 확인한다. |
| 15 | `changed = df[df["vehicle_speed_kmh"] != df["speed_clipped"]]` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 16 | `print(changed[["timestamp", "vehicle_speed_kmh", "speed_clipped"]])` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
