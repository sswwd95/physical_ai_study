# 예제 052 — IQR 기반 이상값 탐지

## 학습 목표
- 자동차 센서 로그에서 IQR 기반 이상값 탐지 절차를 수행합니다.
...
| 10 | `from common.preprocessing import iqr_bounds` | 실습에 필요한 모듈이나 경로 도구를 불러옵니다. |
| 11 | `` | 코드 구간을 구분해 가독성을 높입니다. |
| 12 | `# 전방 거리의 사분위 범위를 계산한다.` | 전방 거리의 사분위 범위를 계산한다. |
| 13 | `df = load_sensor_log()` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 14 | `low, high = iqr_bounds(df["front_distance_m"].dropna())` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 15 | `# IQR 경계 밖의 값을 추출한다.` | IQR 경계 밖의 값을 추출한다. |
| 16 | `mask = ~df["front_distance_m"].between(low, high) & df["front_distance_m"].notna()` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 17 | `print("bounds:", round(low, 2), round(high, 2))` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
| 18 | `print(df.loc[mask, ["timestamp", "front_distance_m"]])` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
