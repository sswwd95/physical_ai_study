...
## 학습 목표
- 자동차 센서 로그에서 전방 채움과 후방 채움 절차를 수행합니다.
...
| 11 | `# 시간순으로 정렬한다.` | 시간순으로 정렬한다. |
| 12 | `df = load_sensor_log().sort_values("timestamp").reset_index(drop=True)` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 13 | `# 배터리 전압은 직전 값으로 채운다.` | 배터리 전압은 직전 값으로 채운다. |
| 14 | `df["battery_v_ffill"] = df["battery_v"].ffill()` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 15 | `# 기어처럼 상태형 값은 앞뒤 채움을 함께 사용할 수 있다.` | 기어처럼 상태형 값은 앞뒤 채움을 함께 사용할 수 있다. |
| 16 | `df["gear_filled"] = df["gear"].ffill().bfill()` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 17 | `print(df[["timestamp", "battery_v", "battery_v_ffill", "gear_filled"]].head(12))` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
