# 예제 044 — 행 삭제와 열 삭제 기준 비교

## 학습 목표
- 자동차 센서 로그에서 행 삭제와 열 삭제 기준 비교 절차를 수행합니다.
...
| 11 | `# 원본 데이터를 읽는다.` | 원본 데이터를 읽는다. |
| 12 | `df = load_sensor_log()` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 13 | `# 결측값이 하나라도 있는 행을 삭제한다.` | 결측값이 하나라도 있는 행을 삭제한다. |
| 14 | `row_dropped = df.dropna()` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 15 | `# 결측률이 5%를 넘는 열만 제거한다.` | 결측률이 5%를 넘는 열만 제거한다. |
| 16 | `keep_columns = df.columns[df.isna().mean() <= 0.05]` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 17 | `column_filtered = df[keep_columns]` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 18 | `print("original:", df.shape)` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
| 19 | `print("drop rows:", row_dropped.shape)` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
| 20 | `print("filter columns:", column_filtered.shape)` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
