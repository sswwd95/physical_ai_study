...
| 6 | `rows=[` | 경로점, 오차, 조향값 또는 평가값을 계산합니다. |
| 7 | `{"controller":"pure_pursuit",**tracking_metrics(simulate_tracker(path,pp,.6,23))},` | 평면 차량 모델로 경로 추종을 시뮬레이션합니다. |
| 8 | `{"controller":"stanley",**tracking_metrics(simulate_tracker(path,st,.6,23))}` | 평면 차량 모델로 경로 추종을 시뮬레이션합니다. |
| 9 | `]` | 현재 경로 추종 절차를 실행합니다. |
| 10 | `r=pd.DataFrame(rows)` | 경로점, 오차, 조향값 또는 평가값을 계산합니다. |
| 11 | `p=output_path("ex333_controller_comparison.csv"); r.to_csv(p,index=False,encoding="utf-8-sig")` | 경로 추종 결과를 저장합니다. |
| 12 | `print(r)` | 목표점, 오차, 성능 또는 저장 경로를 출력합니다. |
