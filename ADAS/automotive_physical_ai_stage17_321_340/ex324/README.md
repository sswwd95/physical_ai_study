...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `from common.path_tracking import load_path, path_heading` | 경로 추종에 필요한 라이브러리와 공통 함수를 불러옵니다. |
| 2 | `path=load_path("path_sine.csv")` | CSV 웨이포인트 경로를 읽습니다. |
| 3 | `for idx in [0,40,100,180,240]:` | 현재 경로 추종 절차를 실행합니다. |
| 4 | `    print(idx,path_heading(path,idx))` | 목표점, 오차, 성능 또는 저장 경로를 출력합니다. |
