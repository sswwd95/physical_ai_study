...
| 6 | `    while viewer.is_running() and data.time<16:` | 현재 자동차 프로젝트 통합 절차를 실행합니다. |
| 7 | `        risk=1/(1+math.exp(-(.45*data.time-4)))` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
| 8 | `        set_wheels(data,7*(1-risk),7*(1-risk))` | 좌우 바퀴 속도 명령을 안전 범위로 제한해 적용합니다. |
| 9 | `        with viewer.lock():` | Viewer와 공유하는 모델 상태를 안전하게 변경합니다. |
| 10 | `            model.geom_rgba[gid]=[risk,1-risk,.1,1]` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
