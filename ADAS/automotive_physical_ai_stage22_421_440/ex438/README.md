...
| 8 | `        raw=np.array([.8,.2])` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
| 9 | `        dist=body_distance(data,base,obs)` | 차량과 장애물 사이의 3차원 거리를 계산합니다. |
| 10 | `        safe=raw.copy()` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
| 11 | `        if dist<1.5:safe[0]=-.8` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
| 12 | `        set_wheels(data,8*safe[0]-4*safe[1],8*safe[0]+4*safe[1])` | 좌우 바퀴 속도 명령을 안전 범위로 제한해 적용합니다. |
| 13 | `        mujoco.mj_step(model,data); viewer.sync(); time.sleep(model.opt.timestep)` | 자동차 물리 시뮬레이션을 한 스텝 진행합니다. |
