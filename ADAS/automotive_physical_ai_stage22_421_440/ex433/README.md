...
| 5 | `    while viewer.is_running() and data.time<16:` | 현재 자동차 프로젝트 통합 절차를 실행합니다. |
| 6 | `        voltage=12.6-.08*data.time` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
| 7 | `        cmd=8 if voltage>12 else 5 if voltage>11.5 else 2` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
| 8 | `        set_wheels(data,cmd,cmd)` | 좌우 바퀴 속도 명령을 안전 범위로 제한해 적용합니다. |
| 9 | `        print("voltage",round(voltage,2),"cmd",cmd)` | 센서값·거리·위험도·결과 경로를 출력합니다. |
| 10 | `        mujoco.mj_step(model,data); viewer.sync(); time.sleep(model.opt.timestep)` | 자동차 물리 시뮬레이션을 한 스텝 진행합니다. |
