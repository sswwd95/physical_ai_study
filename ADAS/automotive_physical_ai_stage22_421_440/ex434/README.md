...
| 6 | `    while viewer.is_running() and data.time<18:` | 현재 자동차 프로젝트 통합 절차를 실행합니다. |
| 7 | `        temp=45+1.8*data.time` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
| 8 | `        cmd=8 if temp<65 else 4 if temp<75 else 0` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
| 9 | `        set_wheels(data,cmd,cmd)` | 좌우 바퀴 속도 명령을 안전 범위로 제한해 적용합니다. |
| 10 | `        with viewer.lock():` | Viewer와 공유하는 모델 상태를 안전하게 변경합니다. |
| 11 | `            model.geom_rgba[gid]=[min(1,(temp-45)/35),.25,.2,1]` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
| 12 | `        mujoco.mj_step(model,data); viewer.sync(); time.sleep(model.opt.timestep)` | 자동차 물리 시뮬레이션을 한 스텝 진행합니다. |
