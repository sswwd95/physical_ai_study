...
| 3 | `df=load_data(); x=standardize(df["speed_mps"]).to_numpy(); y=df["risk_label"].to_numpy()` | 데이터, 모델 모수, 위험확률 또는 평가값을 계산합니다. |
| 4 | `with pm.Model() as model:` | 확률 모델 정의를 시작합니다. |
| 5 | `    a=pm.Normal("intercept",0,2)` | 회귀계수 또는 계층 효과의 정규 사전분포를 정의합니다. |
| 6 | `    b=pm.Normal("speed_coef",0,2)` | 회귀계수 또는 계층 효과의 정규 사전분포를 정의합니다. |
...
| 10 | `s,pth=save_summary(idata,["intercept","speed_coef"],"ex247_speed_logistic.csv")` | 데이터, 모델 모수, 위험확률 또는 평가값을 계산합니다. |
