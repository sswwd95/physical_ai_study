...
| 5 | `    p=pm.Beta("p",alpha=2,beta=8)` | 0~1 위험률에 베타 사전분포를 설정합니다. |
| 6 | `    pm.Bernoulli("obs",p=p,observed=y)` | 이상 여부 관측값의 베르누이 우도를 정의합니다. |
| 7 | `idata=sample_model(model)` | MCMC 샘플링으로 사후분포를 생성합니다. |
| 8 | `s,pth=save_summary(idata,["p"],"ex244_overall_risk_probability.csv")` | 데이터, 모델 모수, 위험확률 또는 평가값을 계산합니다. |
| 9 | `print(s); print("saved:",pth)` | 추정값, 성능, 판정 또는 저장 경로를 출력합니다. |
