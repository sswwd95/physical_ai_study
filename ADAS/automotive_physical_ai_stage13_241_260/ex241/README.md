...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `from common.risk_utils import load_data` | PyMC·ArviZ·데이터 처리 함수를 불러옵니다. |
| 2 | `df=load_data()` | 데이터, 모델 모수, 위험확률 또는 평가값을 계산합니다. |
| 3 | `print(df.head())` | 추정값, 성능, 판정 또는 저장 경로를 출력합니다. |
| 4 | `print(df["risk_label"].value_counts())` | 추정값, 성능, 판정 또는 저장 경로를 출력합니다. |
| 5 | `print("risk ratio:",round(df["risk_label"].mean(),4))` | 추정값, 성능, 판정 또는 저장 경로를 출력합니다. |
