...
| 4 | `for ps in [.02,.1,.5]:` | 현재 분석 절차를 실행합니다. |
| 5 | `    with pm.Model() as model:` | 확률 모델 정의를 시작합니다. |
| 6 | `        bias=pm.Normal("bias",0,ps); sigma=pm.HalfNormal("sigma",.1); pm.Normal("obs",bias,sigma,observed=x)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 7 | `    print(ps,float(sample_model(model).posterior["bias"].mean()))` | MCMC로 사후분포를 샘플링합니다. |
