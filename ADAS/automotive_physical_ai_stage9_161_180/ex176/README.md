...
| 5 | `    bias=pm.Normal("bias",0,.1); sigma=pm.HalfNormal("sigma",.1); nu=pm.Exponential("nu_minus_one",.1)+1; pm.StudentT("obs",nu=nu,mu=bias,sigma=sigma,observed=y)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 6 | `i=sample_model(model); s,p=save_summary(i,["bias","sigma"],"ex176_student_t.csv"); print(s); print(p)` | MCMC로 사후분포를 샘플링합니다. |
