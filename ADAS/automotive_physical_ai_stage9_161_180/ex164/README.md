...
| 5 | `    bias=pm.Normal("bias",0,.1); pm.Normal("obs",bias,.036,observed=x)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 6 | `idata=sample_model(model); s,p=save_summary(idata,["bias"],"ex164_bias_summary.csv"); print(s); print(p)` | MCMC로 사후분포를 샘플링합니다. |
