# 예제 134 — 오도메트리 샘플링 주기 비교

## 학습 목표
TurtleBot3 Burger 차동구동 모델에서 **오도메트리 샘플링 주기 비교** 원리를 이해하고 Python으로 확인합니다.
...
| 2 | `from common.diff_drive import integrate_odometry` | 분석에 필요한 라이브러리와 공통 운동학 함수를 불러옵니다. |
...
| 5 | `for dt in [0.20, 0.10, 0.05, 0.01]:` | 현재 실습 절차를 실행합니다. |
| 6 | `    df = integrate_odometry(commands, dt=dt)` | 속도 명령을 시간에 따라 적분해 로봇 자세를 계산합니다. |
| 7 | `    last = df.iloc[-1]` | 운동학, 오도메트리 또는 오차 분석에 필요한 값을 계산합니다. |
| 8 | `    print("dt=", dt, "final=", round(last.x_m,4), round(last.y_m,4), round(last.yaw_rad,4))` | 핵심 계산값과 저장 경로를 콘솔에 출력합니다. |
