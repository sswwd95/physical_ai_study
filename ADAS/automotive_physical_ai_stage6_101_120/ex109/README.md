# 예제 109 — 초기 상태와 정방향 계산

## 학습 목표
MuJoCo에서 TurtleBot3 Burger 계열 모델을 안전하게 불러오고 **초기 상태와 정방향 계산** 작업을 수행합니다.
...
| 4 | `mujoco.mj_forward(model, data)` | 현재 상태에서 위치·센서 값을 다시 계산합니다. |
| 5 | `print("qpos:", data.qpos.copy())` | 모델 구조나 실행 결과를 콘솔에 출력합니다. |
| 6 | `print("qvel:", data.qvel.copy())` | 모델 구조나 실행 결과를 콘솔에 출력합니다. |
| 7 | `print("sensordata:", data.sensordata.copy())` | 센서 주소와 차원을 이용해 측정값을 읽습니다. |
