...
## 학습 목표
MuJoCo에서 TurtleBot3 Burger 계열 모델을 안전하게 불러오고 **좌회전 차동구동** 작업을 수행합니다.
...
| 4 | `data.ctrl[0] = 4.0` | 좌우 바퀴 액추에이터에 제어 명령을 입력합니다. |
| 5 | `data.ctrl[1] = 8.0` | 좌우 바퀴 액추에이터에 제어 명령을 입력합니다. |
| 6 | `for _ in range(400):` | 현재 실습 절차를 실행합니다. |
| 7 | `    mujoco.mj_step(model, data)` | MuJoCo 물리 시뮬레이션을 한 스텝 진행합니다. |
| 8 | `print("base position:", data.qpos[:3].copy())` | 모델 구조나 실행 결과를 콘솔에 출력합니다. |
| 9 | `print("base quaternion:", data.qpos[3:7].copy())` | 모델 구조나 실행 결과를 콘솔에 출력합니다. |
