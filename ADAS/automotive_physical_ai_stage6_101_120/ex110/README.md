# 예제 110 — 100스텝 헤드리스 시뮬레이션

## 학습 목표
MuJoCo에서 TurtleBot3 Burger 계열 모델을 안전하게 불러오고 **100스텝 헤드리스 시뮬레이션** 작업을 수행합니다.
...
| 4 | `for _ in range(100):` | 현재 실습 절차를 실행합니다. |
| 5 | `    mujoco.mj_step(model, data)` | MuJoCo 물리 시뮬레이션을 한 스텝 진행합니다. |
| 6 | `print("time:", round(data.time, 3))` | 모델 구조나 실행 결과를 콘솔에 출력합니다. |
| 7 | `print("qpos:", data.qpos.copy())` | 모델 구조나 실행 결과를 콘솔에 출력합니다. |
