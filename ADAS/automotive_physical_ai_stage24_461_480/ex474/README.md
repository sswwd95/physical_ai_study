...
| 6 | `        with viewer.lock():` | 현재 교통환경 실습을 수행합니다. |
| 7 | `            intensity=max(.15,1-data.time/16)` | 신호, 거리, 명령 또는 이벤트 값을 계산합니다. |
| 8 | `            model.light_diffuse[0]=[intensity,intensity,intensity]` | 신호, 거리, 명령 또는 이벤트 값을 계산합니다. |
| 9 | `        set_ego(data,6,6)` | 자차의 좌우 바퀴 명령을 적용합니다. |
