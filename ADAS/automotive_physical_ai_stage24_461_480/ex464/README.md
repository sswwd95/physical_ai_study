...
| 6 | `        phase=signal_phase(data.time); x=float(data.qpos[0])` | 현재 교통신호 단계를 계산합니다. |
| 7 | `        distance_to_line=max(0,-x)` | 신호, 거리, 명령 또는 이벤트 값을 계산합니다. |
| 8 | `        cmd=3 if phase=="YELLOW" and distance_to_line>1.5 else 7` | 신호, 거리, 명령 또는 이벤트 값을 계산합니다. |
