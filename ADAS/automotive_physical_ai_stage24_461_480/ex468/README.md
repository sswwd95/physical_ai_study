...
| 6 | `    while viewer.is_running() and data.time<22:` | 현재 교통환경 실습을 수행합니다. |
| 7 | `        lead_x=-2+.3*data.time` | 신호, 거리, 명령 또는 이벤트 값을 계산합니다. |
| 8 | `        data.mocap_pos[0]=[lead_x,.2*.2,.12]` | 교통 참가자의 위치를 시간에 따라 이동시킵니다. |
| 9 | `        gap=dist(pos_xy(data,ego),pos_xy(data,lead))` | 교통 참가자 사이의 거리를 계산합니다. |
| 10 | `        error=gap-2.0` | 신호, 거리, 명령 또는 이벤트 값을 계산합니다. |
| 11 | `        cmd=max(0,min(10,6+2*error))` | 신호, 거리, 명령 또는 이벤트 값을 계산합니다. |
| 12 | `        set_ego(data,cmd,cmd)` | 자차의 좌우 바퀴 명령을 적용합니다. |
| 13 | `        mujoco.mj_step(model,data); viewer.sync(); time.sleep(model.opt.timestep)` | 물리 시뮬레이션을 한 스텝 진행합니다. |
