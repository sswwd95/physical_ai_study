...
| 4 | `ego=body_id(mujoco,model,"ego"); emg=body_id(mujoco,model,"emergency_vehicle")` | 신호, 거리, 명령 또는 이벤트 값을 계산합니다. |
...
| 7 | `        data.mocap_pos[2]=[-8+.45*data.time,-2+.1*data.time,.12]` | 교통 참가자의 위치를 시간에 따라 이동시킵니다. |
| 8 | `        d=dist(pos_xy(data,ego),pos_xy(data,emg))` | 교통 참가자 사이의 거리를 계산합니다. |
| 9 | `        set_ego(data,2,5) if d<3 else set_ego(data,7,7)` | 자차의 좌우 바퀴 명령을 적용합니다. |
