...
| 4 | `manual=False` | 신호, 거리, 명령 또는 이벤트 값을 계산합니다. |
| 5 | `def key(k):` | 현재 교통환경 실습을 수행합니다. |
| 6 | `    global manual` | 현재 교통환경 실습을 수행합니다. |
| 7 | `    if k in (77,109): manual=not manual` | 신호, 거리, 명령 또는 이벤트 값을 계산합니다. |
| 8 | `with mujoco.viewer.launch_passive(model,data,key_callback=key) as viewer:` | 교통 시나리오 제어 루프와 함께 Viewer를 실행합니다. |
| 9 | `    while viewer.is_running():` | 현재 교통환경 실습을 수행합니다. |
| 10 | `        set_ego(data,0,0) if manual else set_ego(data,7,7)` | 자차의 좌우 바퀴 명령을 적용합니다. |
| 11 | `        print("MANUAL" if manual else "AUTO")` | 신호·거리·상태·결과를 출력합니다. |
| 12 | `        mujoco.mj_step(model,data); viewer.sync(); time.sleep(model.opt.timestep)` | 물리 시뮬레이션을 한 스텝 진행합니다. |
