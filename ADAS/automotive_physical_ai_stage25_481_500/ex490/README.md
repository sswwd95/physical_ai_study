...
| 4 | `jid=model.joint("payload_y").id` | 동역학 상태, 위험도 또는 시험 명령을 계산합니다. |
...
| 9 | `            data.qpos[adr]=.12*math.sin(data.time*.6)` | 동역학 상태, 위험도 또는 시험 명령을 계산합니다. |
| 10 | `            mujoco.mj_forward(model,data)` | 현재 차량 동역학 시험 절차를 수행합니다. |
| 11 | `        set_all(data,8); mujoco.mj_step(model,data); viewer.sync(); time.sleep(model.opt.timestep)` | 4개 바퀴 구동 명령을 설정합니다. |
