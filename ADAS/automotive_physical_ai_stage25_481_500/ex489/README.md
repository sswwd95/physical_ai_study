...
| 5 | `    while viewer.is_running() and data.time<18:` | 현재 차량 동역학 시험 절차를 수행합니다. |
| 6 | `        diff=5*math.sin(data.time*1.4)` | 동역학 상태, 위험도 또는 시험 명령을 계산합니다. |
| 7 | `        set_drive(data,10-diff,10+diff,10-diff,10+diff)` | 4개 바퀴 구동 명령을 설정합니다. |
| 8 | `        mujoco.mj_step(model,data)` | 차량 동역학 물리 상태를 한 스텝 진행합니다. |
| 9 | `        print("roll",round(chassis_rpy(data)[0],4))` | 차체의 롤·피치·요 각도를 계산합니다. |
