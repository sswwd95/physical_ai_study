...
| 3 | `env=SimpleCarEnv(max_steps=80,seed=3)` | 교육용 자동차 강화학습 환경을 생성합니다. |
| 4 | `obs,_=env.reset(seed=3)` | 환경을 초기화하고 최초 관측값을 받습니다. |
| 5 | `total=0` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 6 | `for _ in range(80):` | 현재 강화학습 환경 절차를 실행합니다. |
| 7 | `    steering=np.clip(-1.2*obs[1]-0.8*obs[2],-1,1)` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 8 | `    action=np.array([.45,steering],dtype=np.float32)` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 9 | `    obs,reward,terminated,truncated,info=env.step(action)` | 행동을 적용하고 다음 관측·보상·종료상태를 받습니다. |
| 10 | `    total+=reward` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 11 | `    if terminated or truncated: break` | 현재 강화학습 환경 절차를 실행합니다. |
