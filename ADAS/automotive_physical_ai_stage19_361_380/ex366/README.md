...
| 3 | `env=SimpleCarEnv(seed=42)` | 교육용 자동차 강화학습 환경을 생성합니다. |
| 4 | `obs,_=env.reset(seed=42)` | 환경을 초기화하고 최초 관측값을 받습니다. |
| 5 | `next_obs,reward,terminated,truncated,info=env.step(np.array([.5,.1],dtype=np.float32))` | 행동을 적용하고 다음 관측·보상·종료상태를 받습니다. |
| 6 | `print(obs)` | 환경 상태, 보상 또는 검증 결과를 출력합니다. |
| 7 | `print(next_obs,reward,terminated,truncated,info)` | 환경 상태, 보상 또는 검증 결과를 출력합니다. |
| 8 | `env.close()` | 현재 강화학습 환경 절차를 실행합니다. |
