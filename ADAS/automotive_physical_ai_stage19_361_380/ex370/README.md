...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `for distance,error in [(5,.2),(.4,.2),(5,2.6)]:` | 현재 강화학습 환경 절차를 실행합니다. |
| 2 | `    collision=distance<=.5` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 3 | `    lane_departure=abs(error)>=2.5` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 4 | `    terminated=collision or lane_departure` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 5 | `    print(distance,error,terminated)` | 환경 상태, 보상 또는 검증 결과를 출력합니다. |
