...
| 4 | `print("actuator ctrlrange:",model.actuator_ctrlrange.copy())` | 모델 구조, 상태 또는 저장 경로를 출력합니다. |
| 5 | `with mujoco.viewer.launch_passive(model,data) as viewer:` | 제어 루프와 병행하는 passive Viewer를 실행합니다. |
| 6 | `    while viewer.is_running() and data.time<10:` | 현재 Burger Viewer 실습 절차를 수행합니다. |
| 7 | `        set_wheels(data,100,-100)` | 공식 휠 actuator 범위 안에서 좌우 명령을 적용합니다. |
| 8 | `        mujoco.mj_step(model,data)` | MuJoCo 물리 시뮬레이션을 한 스텝 진행합니다. |
| 9 | `        print("applied:",data.ctrl.copy())` | 모델 구조, 상태 또는 저장 경로를 출력합니다. |
| 10 | `        viewer.sync()` | 물리 상태와 Viewer 화면·입력을 동기화합니다. |
| 11 | `        time.sleep(model.opt.timestep)` | 현재 Burger Viewer 실습 절차를 수행합니다. |
