# 예제 510 — Burger base 자유관절 상태 모니터링
...
| 2 | `from common.tb3_burger_utils import load_tb3,set_wheels,base_pose` | 공식 Burger 모델과 Viewer 실행에 필요한 모듈을 불러옵니다. |
| 3 | `mujoco,model,data,ids=load_tb3()` | 공식 scene 파일로 Burger 모델과 이름 ID를 로드합니다. |
| 4 | `next_print=0` | 모델 구조, 상태 또는 저장 경로를 출력합니다. |
...
| 7 | `        set_wheels(data,4,4)` | 공식 휠 actuator 범위 안에서 좌우 명령을 적용합니다. |
| 8 | `        mujoco.mj_step(model,data)` | MuJoCo 물리 시뮬레이션을 한 스텝 진행합니다. |
| 9 | `        if data.time>=next_print:` | 모델 구조, 상태 또는 저장 경로를 출력합니다. |
| 10 | `            print(base_pose(data))` | 모델 구조, 상태 또는 저장 경로를 출력합니다. |
| 11 | `            next_print+=1` | 모델 구조, 상태 또는 저장 경로를 출력합니다. |
| 12 | `        viewer.sync()` | 물리 상태와 Viewer 화면·입력을 동기화합니다. |
| 13 | `        time.sleep(model.opt.timestep)` | 현재 Burger Viewer 실습 절차를 수행합니다. |
