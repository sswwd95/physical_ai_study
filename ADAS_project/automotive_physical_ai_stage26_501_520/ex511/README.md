...
| 4 | `left_dof=model.jnt_dofadr[ids["wheel_left_joint"]]` | 제어값, 상태값, 경로 또는 평가값을 계산합니다. |
| 5 | `right_dof=model.jnt_dofadr[ids["wheel_right_joint"]]` | 제어값, 상태값, 경로 또는 평가값을 계산합니다. |
| 6 | `next_print=0` | 모델 구조, 상태 또는 저장 경로를 출력합니다. |
| 7 | `with mujoco.viewer.launch_passive(model,data) as viewer:` | 제어 루프와 병행하는 passive Viewer를 실행합니다. |
| 8 | `    while viewer.is_running() and data.time<12:` | 현재 Burger Viewer 실습 절차를 수행합니다. |
| 9 | `        set_wheels(data,3.0,5.0)` | 공식 휠 actuator 범위 안에서 좌우 명령을 적용합니다. |
| 10 | `        mujoco.mj_step(model,data)` | MuJoCo 물리 시뮬레이션을 한 스텝 진행합니다. |
| 11 | `        if data.time>=next_print:` | 모델 구조, 상태 또는 저장 경로를 출력합니다. |
| 12 | `            print("left:",data.qvel[left_dof],"right:",data.qvel[right_dof])` | 모델 구조, 상태 또는 저장 경로를 출력합니다. |
| 13 | `            next_print+=1` | 모델 구조, 상태 또는 저장 경로를 출력합니다. |
