# 예제 509 — Tracking Camera Burger 추적
...
| 4 | `    viewer.cam.type=mujoco.mjtCamera.mjCAMERA_TRACKING` | 제어값, 상태값, 경로 또는 평가값을 계산합니다. |
| 5 | `    viewer.cam.trackbodyid=ids["base_body"]` | 제어값, 상태값, 경로 또는 평가값을 계산합니다. |
| 6 | `    viewer.cam.distance=1.8` | 제어값, 상태값, 경로 또는 평가값을 계산합니다. |
| 7 | `    viewer.cam.azimuth=135` | 제어값, 상태값, 경로 또는 평가값을 계산합니다. |
| 8 | `    viewer.cam.elevation=-25` | 제어값, 상태값, 경로 또는 평가값을 계산합니다. |
| 9 | `    realtime_loop(mujoco,model,data,viewer,14,lambda m,d:set_wheels(d,3.5,5.0))` | 공식 휠 actuator 범위 안에서 좌우 명령을 적용합니다. |
