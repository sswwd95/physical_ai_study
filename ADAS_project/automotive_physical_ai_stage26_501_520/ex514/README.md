...
| 4 | `    viewer.opt.flags[mujoco.mjtVisFlag.mjVIS_CONTACTPOINT]=True` | 제어값, 상태값, 경로 또는 평가값을 계산합니다. |
| 5 | `    viewer.opt.flags[mujoco.mjtVisFlag.mjVIS_CONTACTFORCE]=True` | 제어값, 상태값, 경로 또는 평가값을 계산합니다. |
| 6 | `    realtime_loop(mujoco,model,data,viewer,12,lambda m,d:set_wheels(d,5.0,5.0))` | 공식 휠 actuator 범위 안에서 좌우 명령을 적용합니다. |
