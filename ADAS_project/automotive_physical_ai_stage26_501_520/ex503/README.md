...
| 3 | `print("nbody:",model.nbody,"njnt:",model.njnt,"nu:",model.nu)` | 모델 구조, 상태 또는 저장 경로를 출력합니다. |
| 4 | `for key,value in ids.items():` | 현재 Burger Viewer 실습 절차를 수행합니다. |
| 5 | `    print(key,value)` | 모델 구조, 상태 또는 저장 경로를 출력합니다. |
| 6 | `for i in range(model.nu):` | 현재 Burger Viewer 실습 절차를 수행합니다. |
| 7 | `    print("actuator",i,mujoco.mj_id2name(model,mujoco.mjtObj.mjOBJ_ACTUATOR,i))` | 모델 구조, 상태 또는 저장 경로를 출력합니다. |
