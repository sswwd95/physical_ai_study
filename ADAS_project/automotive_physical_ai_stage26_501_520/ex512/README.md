...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `import time,pandas as pd` | 공식 Burger 모델과 Viewer 실행에 필요한 모듈을 불러옵니다. |
...
| 4 | `rows=[]` | 제어값, 상태값, 경로 또는 평가값을 계산합니다. |
| 5 | `with mujoco.viewer.launch_passive(model,data) as viewer:` | 제어 루프와 병행하는 passive Viewer를 실행합니다. |
| 6 | `    while viewer.is_running() and data.time<12:` | 현재 Burger Viewer 실습 절차를 수행합니다. |
| 7 | `        set_wheels(data,4.2,4.8)` | 공식 휠 actuator 범위 안에서 좌우 명령을 적용합니다. |
| 8 | `        mujoco.mj_step(model,data)` | MuJoCo 물리 시뮬레이션을 한 스텝 진행합니다. |
| 9 | `        rows.append({"time_s":float(data.time),**base_pose(data),` | 현재 Burger Viewer 실습 절차를 수행합니다. |
| 10 | `                     "left_ctrl":float(data.ctrl[0]),"right_ctrl":float(data.ctrl[1])})` | 현재 Burger Viewer 실습 절차를 수행합니다. |
| 11 | `        viewer.sync()` | 물리 상태와 Viewer 화면·입력을 동기화합니다. |
| 12 | `        time.sleep(model.opt.timestep)` | 현재 Burger Viewer 실습 절차를 수행합니다. |
| 13 | `p=output_path("ex512_tb3_odometry.csv")` | 제어값, 상태값, 경로 또는 평가값을 계산합니다. |
| 14 | `pd.DataFrame(rows).to_csv(p,index=False,encoding="utf-8-sig")` | 실습 로그 또는 리포트를 저장합니다. |
| 15 | `print(p)` | 모델 구조, 상태 또는 저장 경로를 출력합니다. |
