...
| 5 | `    while viewer.is_running() and data.time<20:` | 현재 자동차 프로젝트 통합 절차를 실행합니다. |
| 6 | `        t=data.time` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
| 7 | `        if t<5:set_wheels(data,6,6)` | 좌우 바퀴 속도 명령을 안전 범위로 제한해 적용합니다. |
| 8 | `        elif t<8:set_wheels(data,14,14)` | 좌우 바퀴 속도 명령을 안전 범위로 제한해 적용합니다. |
| 9 | `        elif t<12:set_wheels(data,-12,12)` | 좌우 바퀴 속도 명령을 안전 범위로 제한해 적용합니다. |
| 10 | `        elif t<16:set_wheels(data,3,10)` | 좌우 바퀴 속도 명령을 안전 범위로 제한해 적용합니다. |
| 11 | `        else:set_wheels(data,0,0)` | 좌우 바퀴 속도 명령을 안전 범위로 제한해 적용합니다. |
