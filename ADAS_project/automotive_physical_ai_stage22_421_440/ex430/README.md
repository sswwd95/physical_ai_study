...
| 6 | `prev=body_distance(data,base,obs)` | 차량과 장애물 사이의 3차원 거리를 계산합니다. |
| 7 | `with mujoco.viewer.launch_passive(model,data) as viewer:` | Python 제어 루프와 함께 실행되는 passive Viewer를 엽니다. |
| 8 | `    while viewer.is_running() and data.time<16:` | 현재 자동차 프로젝트 통합 절차를 실행합니다. |
| 9 | `        dist=body_distance(data,base,obs)` | 차량과 장애물 사이의 3차원 거리를 계산합니다. |
| 10 | `        closing=max(0,(prev-dist)/model.opt.timestep)` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
| 11 | `        ttc=dist/max(closing,.01)` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
| 12 | `        set_wheels(data,0,0) if ttc<1.5 else set_wheels(data,8,8)` | 좌우 바퀴 속도 명령을 안전 범위로 제한해 적용합니다. |
| 13 | `        prev=dist` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
