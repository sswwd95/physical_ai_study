...
| 4 | `next_print=0` | 센서값·거리·위험도·결과 경로를 출력합니다. |
| 5 | `with mujoco.viewer.launch_passive(model,data) as viewer:` | Python 제어 루프와 함께 실행되는 passive Viewer를 엽니다. |
| 6 | `    while viewer.is_running() and data.time<12:` | 현재 자동차 프로젝트 통합 절차를 실행합니다. |
| 7 | `        set_wheels(data,6,7)` | 좌우 바퀴 속도 명령을 안전 범위로 제한해 적용합니다. |
| 8 | `        mujoco.mj_step(model,data)` | 자동차 물리 시뮬레이션을 한 스텝 진행합니다. |
| 9 | `        if data.time>=next_print:` | 센서값·거리·위험도·결과 경로를 출력합니다. |
| 10 | `            print("time",round(data.time,2),"sensordata",data.sensordata.copy())` | 센서값·거리·위험도·결과 경로를 출력합니다. |
| 11 | `            next_print+=1` | 센서값·거리·위험도·결과 경로를 출력합니다. |
