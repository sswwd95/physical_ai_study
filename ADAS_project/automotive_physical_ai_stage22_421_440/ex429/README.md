...
| 7 | `    while viewer.is_running() and data.time<16:` | 현재 자동차 프로젝트 통합 절차를 실행합니다. |
| 8 | `        dist=body_distance(data,base,obs)` | 차량과 장애물 사이의 3차원 거리를 계산합니다. |
| 9 | `        speed=8 if dist>2 else 4 if dist>1 else 0` | 제어값, 상태값, 위험도 또는 진단값을 계산합니다. |
| 10 | `        set_wheels(data,speed,speed)` | 좌우 바퀴 속도 명령을 안전 범위로 제한해 적용합니다. |
| 11 | `        mujoco.mj_step(model,data)` | 자동차 물리 시뮬레이션을 한 스텝 진행합니다. |
| 12 | `        print("distance",round(dist,3),"command",speed)` | 센서값·거리·위험도·결과 경로를 출력합니다. |
