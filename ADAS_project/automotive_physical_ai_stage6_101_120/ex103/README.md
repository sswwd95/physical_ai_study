...
## 학습 목표
MuJoCo에서 TurtleBot3 Burger 계열 모델을 안전하게 불러오고 **XML 모델 로딩** 작업을 수행합니다.
...
| 2 | `path, model, data = load_model_and_data()` | XML 모델을 읽고 시뮬레이션 상태 객체를 만듭니다. |
| 3 | `print("model:", path)` | 모델 구조나 실행 결과를 콘솔에 출력합니다. |
| 4 | `print("nq:", model.nq, "nv:", model.nv, "nu:", model.nu, "nsensor:", model.nsensor)` | 모델 구조나 실행 결과를 콘솔에 출력합니다. |
| 5 | `print("simulation time:", data.time)` | 모델 구조나 실행 결과를 콘솔에 출력합니다. |
