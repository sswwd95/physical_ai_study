...
## 학습 목표
MuJoCo에서 TurtleBot3 Burger 계열 모델을 안전하게 불러오고 **양쪽 바퀴 정회전 제어** 작업을 수행합니다.
...
| 6 | `data.ctrl[0] = 8.0` | 좌우 바퀴 액추에이터에 제어 명령을 입력합니다. |
| 7 | `data.ctrl[1] = 8.0` | 좌우 바퀴 액추에이터에 제어 명령을 입력합니다. |
| 8 | `for _ in range(300):` | 현재 실습 절차를 실행합니다. |
| 9 | `    mujoco.mj_step(model, data)` | MuJoCo 물리 시뮬레이션을 한 스텝 진행합니다. |
| 10 | `print("time:", round(data.time, 3))` | 모델 구조나 실행 결과를 콘솔에 출력합니다. |
| 11 | `print("base position:", data.qpos[:3].copy())` | 모델 구조나 실행 결과를 콘솔에 출력합니다. |
| 12 | `print("controls:", data.ctrl.copy())` | 좌우 바퀴 액추에이터에 제어 명령을 입력합니다. |
