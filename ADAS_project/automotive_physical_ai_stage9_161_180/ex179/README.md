...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `import pymc as pm, arviz as az` | 필요한 PyMC·ArviZ·공통 함수를 불러옵니다. |
...
| 5 | `from common.bayes_utils import load_data,sample_model,output_path` | 필요한 PyMC·ArviZ·공통 함수를 불러옵니다. |
| 6 | `x=load_data()["accel_measurement_mps2"].to_numpy()` | 관측값, 모수, 통계량 또는 판정값을 계산합니다. |
| 7 | `with pm.Model() as model:` | 확률 모델 정의를 시작합니다. |
| 8 | `    bias=pm.Normal("bias",0,.1); sigma=pm.HalfNormal("sigma",.1); pm.Normal("obs",bias,sigma,observed=x)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 9 | `i=sample_model(model); az.plot_posterior(i,var_names=["bias","sigma"]); p=output_path("ex179_posterior.png"); plt.savefig(p,dpi=140); plt.close("all"); print(p)` | MCMC로 사후분포를 샘플링합니다. |
