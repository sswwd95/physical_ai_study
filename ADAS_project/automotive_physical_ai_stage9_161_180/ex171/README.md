...
| 5 | `    ma=pm.Normal("mu_a",0,.1); mb=pm.Normal("mu_b",0,.1); sa=pm.HalfNormal("sigma_a",.1); sb=pm.HalfNormal("sigma_b",.1); delta=pm.Deterministic("delta",mb-ma); pm.Normal("obs_a",ma,sa,observed=a); pm.Normal("obs_b",mb,sb,observed=b)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 6 | `idata=sample_model(model); s,p=save_summary(idata,["mu_a","mu_b","delta"],"ex171_sensor_compare.csv"); print(s); print(p)` | MCMC로 사후분포를 샘플링합니다. |
