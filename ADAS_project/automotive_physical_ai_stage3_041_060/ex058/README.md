# 예제 058 — 급변률 기반 이상값 탐지

## 학습 목표
- 자동차 센서 로그에서 급변률 기반 이상값 탐지 절차를 수행합니다.
...
| 11 | `# 시간순 속도 로그를 준비한다.` | 시간순 속도 로그를 준비한다. |
| 12 | `df = load_sensor_log().sort_values("timestamp").drop_duplicates("timestamp").reset_index(drop=True)` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 13 | `# 0.1초 간격에서 속도 차이를 가속도로 환산한다.` | 0.1초 간격에서 속도 차이를 가속도로 환산한다. |
| 14 | `df["accel_est_mps2"] = df["vehicle_speed_kmh"].diff().div(3.6).div(0.1)` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 15 | `# 승용차 데이터에서 절댓값 8m/s² 초과를 급격한 변화로 표시한다.` | 승용차 데이터에서 절댓값 8m/s² 초과를 급격한 변화로 표시한다. |
| 16 | `mask = df["accel_est_mps2"].abs() > 8` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 17 | `print(df.loc[mask, ["timestamp", "vehicle_speed_kmh", "accel_est_mps2"]])` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
