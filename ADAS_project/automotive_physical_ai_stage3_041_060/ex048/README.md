...
## 학습 목표
- 자동차 센서 로그에서 평균·중앙값 대체 비교 절차를 수행합니다.
...
| 11 | `# 데이터를 읽는다.` | 데이터를 읽는다. |
| 12 | `df = load_sensor_log()` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 13 | `# 조향각 결측값을 평균과 중앙값으로 각각 대체한다.` | 조향각 결측값을 평균과 중앙값으로 각각 대체한다. |
| 14 | `mean_value = df["steering_deg"].mean()` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 15 | `median_value = df["steering_deg"].median()` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 16 | `df["steering_mean_fill"] = df["steering_deg"].fillna(mean_value)` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 17 | `df["steering_median_fill"] = df["steering_deg"].fillna(median_value)` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 18 | `print("mean:", round(mean_value, 3), "median:", round(median_value, 3))` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
