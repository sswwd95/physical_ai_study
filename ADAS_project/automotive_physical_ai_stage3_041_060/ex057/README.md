# 예제 057 — 센서 지연 모사와 시간 이동

## 학습 목표
- 자동차 센서 로그에서 센서 지연 모사와 시간 이동 절차를 수행합니다.
...
| 11 | `# 시간순 로그를 준비한다.` | 시간순 로그를 준비한다. |
| 12 | `df = load_sensor_log().sort_values("timestamp").drop_duplicates("timestamp").reset_index(drop=True)` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 13 | `# 카메라 거리 센서가 2프레임 늦게 도착했다고 가정한다.` | 카메라 거리 센서가 2프레임 늦게 도착했다고 가정한다. |
| 14 | `df["distance_delayed"] = df["front_distance_m"].shift(2)` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 15 | `# 지연 보정을 위해 2프레임 앞으로 이동한다.` | 지연 보정을 위해 2프레임 앞으로 이동한다. |
| 16 | `df["distance_aligned"] = df["distance_delayed"].shift(-2)` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 17 | `print(df[["front_distance_m", "distance_delayed", "distance_aligned"]].head(8))` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
