# 예제 043 — 결측률 계산과 우선순위 결정

## 학습 목표
- 자동차 센서 로그에서 결측률 계산과 우선순위 결정 절차를 수행합니다.
...
| 11 | `# 센서 로그를 읽는다.` | 센서 로그를 읽는다. |
| 12 | `df = load_sensor_log()` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 13 | `# 각 열의 결측 비율을 백분율로 계산한다.` | 각 열의 결측 비율을 백분율로 계산한다. |
| 14 | `missing_rate = df.isna().mean().mul(100).sort_values(ascending=False)` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 15 | `# 결측률이 0보다 큰 열만 출력한다.` | 결측률이 0보다 큰 열만 출력한다. |
| 16 | `print(missing_rate[missing_rate > 0].round(2))` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
| 17 | `# 1% 이상이면 우선 점검 대상으로 표시한다.` | 1% 이상이면 우선 점검 대상으로 표시한다. |
| 18 | `priority = missing_rate[missing_rate >= 1.0]` | 처리 결과를 변수나 데이터 열에 저장합니다. |
| 19 | `print("priority columns:", priority.index.tolist())` | 중간 결과 또는 검증 결과를 콘솔에 출력합니다. |
