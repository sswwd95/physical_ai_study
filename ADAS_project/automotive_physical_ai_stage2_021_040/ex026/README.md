...
## 라인별 해설
- **01행** `def mps_to_kph(speed_mps: float) -> float:` — 반복 사용할 기능을 함수로 정의합니다.
- **02행** `return speed_mps * 3.6` — 이 줄의 명령을 실행합니다.
- **04행** `for speed in [0.0, 5.0, 10.0, 13.9]:` — 여러 센서 샘플을 순서대로 반복 처리합니다.
- **05행** `print(f"{speed:4.1f} m/s -> {mps_to_kph(speed):5.1f} km/h")` — 계산 결과나 상태를 화면에 출력해 확인합니다.
