...
- **04행** `print(df.dtypes)` — 계산 결과나 상태를 화면에 출력해 확인합니다.
- **05행** `print("` — 계산 결과나 상태를 화면에 출력해 확인합니다.
- **06행** `수치형 요약:` — 이 줄의 명령을 실행합니다.
- **07행** `", df.describe().round(3))` — 이 줄의 명령을 실행합니다.
- **08행** `print("` — 계산 결과나 상태를 화면에 출력해 확인합니다.
- **09행** `결측값 수:` — 이 줄의 명령을 실행합니다.
- **10행** `", df.isna().sum())` — 이 줄의 명령을 실행합니다.
