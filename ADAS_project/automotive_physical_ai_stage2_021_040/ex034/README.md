...
- **04행** `condition = (df["motor_temp_c"] > 31.5) & (df["battery_v"] < 12.3)` — 센서 값, 조건식 또는 계산 결과를 변수에 저장합니다.
- **05행** `result = df.loc[condition, ["time_s", "battery_v", "motor_temp_c", "status"]]` — 센서 값, 조건식 또는 계산 결과를 변수에 저장합니다.
- **06행** `print(result.head(20))` — 계산 결과나 상태를 화면에 출력해 확인합니다.
- **07행** `print("조건 충족 행 수:", len(result))` — 계산 결과나 상태를 화면에 출력해 확인합니다.
