# 실습 037 | 이동평균으로 노이즈 완화

## 핵심 주제
속도 센서에 5샘플 이동평균을 적용한다.
...
- **04행** `df["speed_ma5"] = df["speed_mps"].rolling(window=5, min_periods=1).mean()` — 센서 값, 조건식 또는 계산 결과를 변수에 저장합니다.
- **05행** `print(df[["time_s", "speed_mps", "speed_ma5"]].head(15).round(3))` — 계산 결과나 상태를 화면에 출력해 확인합니다.
