...
| 4 | `y = df.loc[df["surface"]=="dry", "slip_ratio"].to_numpy()` | 데이터, 모델 모수, 통계량 또는 판정값을 계산합니다. |
| 5 | `with pm.Model() as model:` | 확률 모델 정의를 시작합니다. |
| 6 | `    mu = pm.Normal("mu", mu=0.03, sigma=0.05)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 7 | `    sigma = pm.HalfNormal("sigma", sigma=0.05)` | 양수인 표준편차의 사전분포를 정의합니다. |
| 8 | `    pm.Normal("obs", mu=mu, sigma=sigma, observed=y)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 9 | `idata = sample_model(model)` | MCMC 샘플링으로 사후분포를 생성합니다. |
| 10 | `summary, path = save_summary(idata, ["mu","sigma"], "ex185_dry_surface.csv")` | 데이터, 모델 모수, 통계량 또는 판정값을 계산합니다. |
| 11 | `print(summary)` | 핵심 추정값·진단값·저장 경로를 출력합니다. |
| 12 | `print("saved:", path)` | 핵심 추정값·진단값·저장 경로를 출력합니다. |
