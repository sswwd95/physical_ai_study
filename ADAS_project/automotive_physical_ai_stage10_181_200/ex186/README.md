...
| 4 | `y = df.loc[df["surface"]=="wet", "slip_ratio"].to_numpy()` | 데이터, 모델 모수, 통계량 또는 판정값을 계산합니다. |
| 5 | `with pm.Model() as model:` | 확률 모델 정의를 시작합니다. |
| 6 | `    mu = pm.Normal("mu", mu=0.10, sigma=0.08)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
...
| 10 | `summary, path = save_summary(idata, ["mu","sigma"], "ex186_wet_surface.csv")` | 데이터, 모델 모수, 통계량 또는 판정값을 계산합니다. |
