...
| 3 | `y = load_data()["distance_error_m"].to_numpy()` | 데이터, 모델 모수, 통계량 또는 판정값을 계산합니다. |
| 4 | `with pm.Model() as model:` | 확률 모델 정의를 시작합니다. |
| 5 | `    bias = pm.Normal("bias", 0.0, 0.20)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 6 | `    sigma = pm.HalfNormal("sigma", 0.20)` | 양수인 표준편차의 사전분포를 정의합니다. |
| 7 | `    pm.StudentT("obs", nu=4, mu=bias, sigma=sigma, observed=y)` | 이상값에 강건한 Student-t 우도를 정의합니다. |
| 8 | `idata = sample_model(model)` | MCMC 샘플링으로 사후분포를 생성합니다. |
| 9 | `summary, path = save_summary(idata, ["bias","sigma"], "ex196_robust_distance_error.csv")` | 데이터, 모델 모수, 통계량 또는 판정값을 계산합니다. |
| 10 | `print(summary)` | 핵심 추정값·진단값·저장 경로를 출력합니다. |
| 11 | `print("saved:", path)` | 핵심 추정값·진단값·저장 경로를 출력합니다. |
