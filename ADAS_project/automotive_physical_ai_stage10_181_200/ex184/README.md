...
| 3 | `y = load_data()["slip_ratio"].to_numpy()` | 데이터, 모델 모수, 통계량 또는 판정값을 계산합니다. |
| 4 | `with pm.Model() as model:` | 확률 모델 정의를 시작합니다. |
| 5 | `    mu = pm.Normal("mu", mu=0.08, sigma=0.08)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 6 | `    sigma = pm.HalfNormal("sigma", sigma=0.08)` | 양수인 표준편차의 사전분포를 정의합니다. |
| 7 | `    pm.Normal("obs", mu=mu, sigma=sigma, observed=y)` | 정규 사전분포 또는 정규 우도를 정의합니다. |
| 8 | `idata = sample_model(model)` | MCMC 샘플링으로 사후분포를 생성합니다. |
| 9 | `summary, path = save_summary(idata, ["mu","sigma"], "ex184_slip_mean_sigma.csv")` | 데이터, 모델 모수, 통계량 또는 판정값을 계산합니다. |
