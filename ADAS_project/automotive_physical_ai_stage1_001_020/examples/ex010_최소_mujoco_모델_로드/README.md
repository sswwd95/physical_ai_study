# 예제 010 — 최소 MuJoCo 모델 로드

## 핵심 주제
내장 MJCF를 로드하고 100스텝 전진 계산한다.
...
cd /d <압축을_푼_폴더>
python examples\ex010_최소_mujoco_모델_로드\main.py
```
...
| 7 | `for _ in range(100):` | 여러 항목 또는 여러 시뮬레이션 스텝을 반복합니다. |
| 8 | `    mujoco.mj_step(model, data)` | 물리 시뮬레이션을 한 타임스텝 전진시킵니다. |
| 9 | `print("time:", data.time)` | 실행 결과나 진단 정보를 화면에 출력합니다. |
| 10 | `print("qpos:", data.qpos.copy())` | 실행 결과나 진단 정보를 화면에 출력합니다. |
| 11 | `print("sensordata:", data.sensordata.copy())` | 실행 결과나 진단 정보를 화면에 출력합니다. |
