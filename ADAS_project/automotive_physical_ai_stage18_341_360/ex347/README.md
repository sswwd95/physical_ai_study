...
| 3 | `df["unsafe_distance"]=df["distance_m"]<df["safe_distance_m"]` | 반응거리·제동거리·여유거리를 합산합니다. |
| 4 | `p=output_path("ex347_unsafe_distance.csv")` | 반응거리·제동거리·여유거리를 합산합니다. |
| 5 | `df[df["unsafe_distance"]].to_csv(p,index=False,encoding="utf-8-sig")` | 반응거리·제동거리·여유거리를 합산합니다. |
| 6 | `print("unsafe samples:",int(df["unsafe_distance"].sum()))` | 반응거리·제동거리·여유거리를 합산합니다. |
