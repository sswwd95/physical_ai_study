...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `from common.safety_utils import safe_distance` | 안전거리·충돌 위험 분석에 필요한 라이브러리를 불러옵니다. |
| 2 | `for speed in [3,5,8,12]:` | 현재 안전 제어 절차를 실행합니다. |
| 3 | `    print(speed,safe_distance(speed,reaction_time=1.2,friction=.7,margin=2))` | 반응거리·제동거리·여유거리를 합산합니다. |
