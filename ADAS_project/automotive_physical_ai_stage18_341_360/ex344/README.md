...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `from common.safety_utils import braking_distance` | 안전거리·충돌 위험 분석에 필요한 라이브러리를 불러옵니다. |
| 2 | `for speed in [3,5,8,12,16]:` | 현재 안전 제어 절차를 실행합니다. |
| 3 | `    print(speed,braking_distance(speed,friction=.7))` | 속도와 마찰계수로 제동거리를 계산합니다. |
