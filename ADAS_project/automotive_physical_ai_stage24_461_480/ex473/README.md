...
| 7 | `        set_ego(data,3,8) if 1.5<x<3.5 else set_ego(data,7,7)` | 자차의 좌우 바퀴 명령을 적용합니다. |
| 8 | `        mujoco.mj_step(model,data); viewer.sync(); time.sleep(model.opt.timestep)` | 물리 시뮬레이션을 한 스텝 진행합니다. |
