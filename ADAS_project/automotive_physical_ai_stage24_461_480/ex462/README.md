...
| 2 | `from common.traffic_utils import load_project,signal_phase` | 교통·V2X Viewer용 모듈을 불러옵니다. |
...
| 6 | `        print("phase:",signal_phase(data.time))` | 현재 교통신호 단계를 계산합니다. |
| 7 | `        mujoco.mj_step(model,data); viewer.sync(); time.sleep(model.opt.timestep)` | 물리 시뮬레이션을 한 스텝 진행합니다. |
