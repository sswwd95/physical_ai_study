...
| 5 | `    while viewer.is_running() and data.time<16:` | 현재 교통환경 실습을 수행합니다. |
| 6 | `        visibility=max(5,25-1.2*data.time)` | 신호, 거리, 명령 또는 이벤트 값을 계산합니다. |
| 7 | `        cmd=7 if visibility>15 else 4 if visibility>8 else 2` | 신호, 거리, 명령 또는 이벤트 값을 계산합니다. |
| 8 | `        set_ego(data,cmd,cmd)` | 자차의 좌우 바퀴 명령을 적용합니다. |
| 9 | `        print("visibility_m",round(visibility,1))` | 신호·거리·상태·결과를 출력합니다. |
