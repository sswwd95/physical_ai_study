...
| 5 | `    a=pm.Normal("intercept",0,2); b=pm.Normal("ttc_coef",0,2)` | 회귀계수 또는 계층 효과의 정규 사전분포를 정의합니다. |
| 6 | `    p=pm.math.sigmoid(a+b*x); pm.Bernoulli("obs",p=p,observed=y)` | 이상 여부 관측값의 베르누이 우도를 정의합니다. |
| 7 | `idata=sample_model(model)` | MCMC 샘플링으로 사후분포를 생성합니다. |
| 8 | `s=az.summary(idata,var_names=["intercept","ttc_coef"],round_to=6)` | 데이터, 모델 모수, 위험확률 또는 평가값을 계산합니다. |
| 9 | `pth=output_path("ex258_diagnostics.csv"); s.to_csv(pth,encoding="utf-8-sig")` | 분석 결과를 outputs 폴더에 저장합니다. |
| 10 | `print(s[["r_hat","ess_bulk","ess_tail"]]); print(pth)` | 추정값, 성능, 판정 또는 저장 경로를 출력합니다. |
