...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `from common.reliability_utils import load_lifetime` | PyMC·ArviZ·신뢰성 분석 함수를 불러옵니다. |
| 2 | `df=load_lifetime()` | 수명 또는 RUL 데이터를 읽습니다. |
| 3 | `print(df.head())` | 추정값, 진단값 또는 저장 경로를 출력합니다. |
| 4 | `print(df.groupby("component")["failure_event"].agg(["count","sum","mean"]))` | 추정값, 진단값 또는 저장 경로를 출력합니다. |
