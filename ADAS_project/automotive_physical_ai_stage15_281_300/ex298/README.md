...
| 2 | `from common.reliability_utils import load_rul,save_json` | PyMC·ArviZ·신뢰성 분석 함수를 불러옵니다. |
| 3 | `df=load_rul()` | 수명 또는 RUL 데이터를 읽습니다. |
| 4 | `threshold=200` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
| 5 | `prob=float(np.mean(df["observed_rul_h"]<threshold))` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
| 6 | `result={"threshold_h":threshold,"empirical_probability_rul_below_threshold":prob}` | 수명, 생존확률, RUL 또는 평가값을 계산합니다. |
| 7 | `print(result); print(save_json(result,"ex298_maintenance_threshold_probability.json"))` | 추정값, 진단값 또는 저장 경로를 출력합니다. |
