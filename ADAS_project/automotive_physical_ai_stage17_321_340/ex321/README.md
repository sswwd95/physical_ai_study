...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `from common.path_tracking import load_path` | 경로 추종에 필요한 라이브러리와 공통 함수를 불러옵니다. |
| 2 | `path = load_path("path_sine.csv")` | CSV 웨이포인트 경로를 읽습니다. |
| 3 | `print(path.head())` | 목표점, 오차, 성능 또는 저장 경로를 출력합니다. |
| 4 | `print(path.tail())` | 목표점, 오차, 성능 또는 저장 경로를 출력합니다. |
| 5 | `print("waypoints:", len(path))` | 목표점, 오차, 성능 또는 저장 경로를 출력합니다. |
