...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `from common.path_tracking import load_path,stanley_control` | 경로 추종에 필요한 라이브러리와 공통 함수를 불러옵니다. |
| 2 | `path=load_path("path_sine.csv")` | CSV 웨이포인트 경로를 읽습니다. |
| 3 | `print(stanley_control(path,x=0,y=-.7,yaw=0,speed=.6,gain=1.2))` | 방향오차와 횡오차 기반 조향 명령을 계산합니다. |
