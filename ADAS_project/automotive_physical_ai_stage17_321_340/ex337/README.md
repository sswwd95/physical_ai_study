...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `from common.path_tracking import load_path,nearest_path_index,signed_cross_track_error` | 경로 추종에 필요한 라이브러리와 공통 함수를 불러옵니다. |
| 2 | `path=load_path("path_sine.csv")` | CSV 웨이포인트 경로를 읽습니다. |
| 3 | `for x,y in [(1,.2),(3,2.4),(7,-2.0)]:` | 현재 경로 추종 절차를 실행합니다. |
| 4 | `    idx=nearest_path_index(path,x,y)` | 차량과 가장 가까운 경로점을 찾습니다. |
| 5 | `    e=signed_cross_track_error(path,idx,x,y)` | 경로점, 오차, 조향값 또는 평가값을 계산합니다. |
| 6 | `    mode="RECOVERY" if abs(e)>1.0 else "TRACKING"` | 경로점, 오차, 조향값 또는 평가값을 계산합니다. |
| 7 | `    print(x,y,e,mode)` | 목표점, 오차, 성능 또는 저장 경로를 출력합니다. |
