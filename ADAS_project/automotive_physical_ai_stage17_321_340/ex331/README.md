...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `from common.path_tracking import load_path,stanley_control,simulate_tracker,tracking_metrics,output_path` | 경로 추종에 필요한 라이브러리와 공통 함수를 불러옵니다. |
| 2 | `path=load_path("path_sine.csv")` | CSV 웨이포인트 경로를 읽습니다. |
| 3 | `def controller(path,x,y,yaw,speed):` | 현재 경로 추종 절차를 실행합니다. |
| 4 | `    return stanley_control(path,x,y,yaw,speed,gain=1.2)` | 방향오차와 횡오차 기반 조향 명령을 계산합니다. |
| 5 | `df=simulate_tracker(path,controller,.6,23)` | 평면 차량 모델로 경로 추종을 시뮬레이션합니다. |
| 6 | `p=output_path("ex331_stanley.csv"); df.to_csv(p,index=False,encoding="utf-8-sig")` | 경로 추종 결과를 저장합니다. |
