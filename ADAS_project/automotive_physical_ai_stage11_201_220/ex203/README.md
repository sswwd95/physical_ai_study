...
## 학습 목표
자동차 센서 로그에서 **급가속 탐지** 절차를 수행하고 ROS2 실시간 진단으로 연결하는 기초를 익힙니다.

## 실행
```bat
cd /d C:\work\automotive_physical_ai_stage11_201_220
conda activate auto_physical_ai
python ex203\main.py
```
...
| 3 | `events = df[df["accel_mps2"] > 1.8].copy()` | 주행 상태, 이상 조건, 위험 점수 또는 평가값을 계산합니다. |
| 4 | `path = output_path("ex203_hard_acceleration.csv")` | 주행 상태, 이상 조건, 위험 점수 또는 평가값을 계산합니다. |
...
| 7 | `print(events[["time_s","accel_mps2","event_label"]].head())` | 탐지 개수, 임계값, 평가 지표 또는 저장 경로를 출력합니다. |
