# 예제 201 — 주행 이상 데이터 구조 확인

## 학습 목표
자동차 센서 로그에서 **주행 이상 데이터 구조 확인** 절차를 수행하고 ROS2 실시간 진단으로 연결하는 기초를 익힙니다.
...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `from common.anomaly_utils import load_data` | 주행 분석에 필요한 라이브러리와 공통 함수를 불러옵니다. |
| 2 | `df = load_data()` | 합성 자동차 주행 이상 로그를 읽습니다. |
| 3 | `print(df.head())` | 탐지 개수, 임계값, 평가 지표 또는 저장 경로를 출력합니다. |
| 4 | `print(df["event_label"].value_counts())` | 탐지 개수, 임계값, 평가 지표 또는 저장 경로를 출력합니다. |
| 5 | `print("rows:", len(df))` | 탐지 개수, 임계값, 평가 지표 또는 저장 경로를 출력합니다. |
