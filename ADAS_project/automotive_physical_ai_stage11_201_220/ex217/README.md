# 예제 217 — 규칙 기반과 Isolation Forest 비교

## 학습 목표
자동차 센서 로그에서 **규칙 기반과 Isolation Forest 비교** 절차를 수행하고 ROS2 실시간 진단으로 연결하는 기초를 익힙니다.

## 실행
```bat
cd /d C:\work\automotive_physical_ai_stage11_201_220
conda activate auto_physical_ai
python ex217\main.py
```
...
| 2 | `from common.anomaly_utils import load_data, confusion_counts` | 주행 분석에 필요한 라이브러리와 공통 함수를 불러옵니다. |
| 3 | `df = load_data()` | 합성 자동차 주행 이상 로그를 읽습니다. |
| 4 | `truth = df["event_label"] != "NORMAL"` | 주행 상태, 이상 조건, 위험 점수 또는 평가값을 계산합니다. |
| 5 | `rule = (` | 주행 상태, 이상 조건, 위험 점수 또는 평가값을 계산합니다. |
...
| 11 | `features = df[["speed_mps","accel_mps2","steering_deg","ttc_s","motor_current_a"]]` | 주행 상태, 이상 조건, 위험 점수 또는 평가값을 계산합니다. |
| 12 | `iso = IsolationForest(contamination=0.10, random_state=42).fit_predict(features) == -1` | 비지도 이상 탐지 모델을 생성하거나 학습합니다. |
| 13 | `print("rule:", confusion_counts(truth,rule))` | 정답과 예측을 비교해 TP·FP·TN·FN을 계산합니다. |
| 14 | `print("isolation_forest:", confusion_counts(truth,iso))` | 정답과 예측을 비교해 TP·FP·TN·FN을 계산합니다. |
