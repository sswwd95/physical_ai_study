...
| 3 | `action_space=spaces.Box(` | 행동 또는 관측 공간의 범위와 형태를 확인합니다. |
| 4 | `    low=np.array([-1,-1],dtype=np.float32),` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 5 | `    high=np.array([1,1],dtype=np.float32))` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 6 | `print(action_space)` | 행동 또는 관측 공간의 범위와 형태를 확인합니다. |
| 7 | `print("sample:",action_space.sample())` | 행동 또는 관측 공간의 범위와 형태를 확인합니다. |
