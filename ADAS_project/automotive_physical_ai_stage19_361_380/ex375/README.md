...
| 2 | `def safety_filter(action,obstacle_distance):` | 현재 강화학습 환경 절차를 실행합니다. |
| 3 | `    action=np.asarray(action,dtype=np.float32).copy()` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 4 | `    if obstacle_distance<3.0:` | 현재 강화학습 환경 절차를 실행합니다. |
| 5 | `        action[0]=min(action[0],-.5)` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 6 | `    return np.clip(action,-1,1)` | 현재 강화학습 환경 절차를 실행합니다. |
| 7 | `for d in [8,2.8,1.2]:` | 현재 강화학습 환경 절차를 실행합니다. |
| 8 | `    print(d,safety_filter([.8,.2],d))` | 환경 상태, 보상 또는 검증 결과를 출력합니다. |
