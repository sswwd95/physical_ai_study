...
| 줄 | 소스 | 설명 |
|---:|---|---|
| 1 | `from common.rl_utils import normalize,denormalize` | Gymnasium 환경과 분석에 필요한 모듈을 불러옵니다. |
| 2 | `speed=1.2` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 3 | `normalized=normalize(speed,0.0,2.0)` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 4 | `restored=denormalize(normalized,0.0,2.0)` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 5 | `print(speed,normalized,restored)` | 환경 상태, 보상 또는 검증 결과를 출력합니다. |
