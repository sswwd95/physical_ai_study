...
| 2 | `for action in [np.array([0,0]),np.array([.5,.2]),np.array([1,-1])]:` | 현재 강화학습 환경 절차를 실행합니다. |
| 3 | `    cost=.05*float(np.sum(action**2))` | 상태, 행동, 보상 또는 통계값을 계산합니다. |
| 4 | `    print(action,cost)` | 환경 상태, 보상 또는 검증 결과를 출력합니다. |
