...
| 2 | `from stable_baselines3.common.env_util import make_vec_env` | Stable-Baselines3·Gymnasium·분석 모듈을 불러옵니다. |
| 3 | `from envs.simple_car_env import SimpleCarEnv` | Stable-Baselines3·Gymnasium·분석 모듈을 불러옵니다. |
| 4 | `vec_env=make_vec_env(SimpleCarEnv,n_envs=4,seed=42,env_kwargs={"max_steps":100})` | 모델, 환경, 행동, 보상 또는 통계값을 계산합니다. |
| 5 | `model=PPO("MlpPolicy",vec_env,verbose=0,seed=42,n_steps=128,batch_size=128)` | PPO 정책과 학습 하이퍼파라미터를 설정합니다. |
| 6 | `model.learn(total_timesteps=1200)` | 지정한 timesteps 동안 PPO 정책을 학습합니다. |
| 7 | `print("vector training complete")` | 학습 상태, 평가 결과 또는 저장 경로를 출력합니다. |
| 8 | `vec_env.close()` | 현재 PPO 학습·평가 절차를 실행합니다. |
