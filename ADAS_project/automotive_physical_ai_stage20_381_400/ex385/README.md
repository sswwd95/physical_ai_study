...
| 4 | `env=SimpleCarEnv(max_steps=100)` | 모델, 환경, 행동, 보상 또는 통계값을 계산합니다. |
| 5 | `model=PPO("MlpPolicy",env,verbose=0,seed=42,n_steps=128,batch_size=64)` | PPO 정책과 학습 하이퍼파라미터를 설정합니다. |
| 6 | `model.learn(total_timesteps=800)` | 지정한 timesteps 동안 PPO 정책을 학습합니다. |
| 7 | `p=model_path("ex385_ppo_car")` | 모델, 환경, 행동, 보상 또는 통계값을 계산합니다. |
| 8 | `model.save(str(p))` | 학습 모델을 저장하거나 다시 불러옵니다. |
| 9 | `print("saved:",p)` | 학습 상태, 평가 결과 또는 저장 경로를 출력합니다. |
