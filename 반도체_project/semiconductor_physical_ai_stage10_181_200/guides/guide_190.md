...
python examples\ex190_logistic_coefficient_probability.py
```
...
| 14 | `X=defect_df[features]; X=(X-X.mean())/X.std(); y=(defect_df["defect_count"]>0).astype(int)` | 계산 결과나 설정값을 변수에 저장합니다. |
...
| 17 | `    pm.Bernoulli("y",logit_p=a+pm.math.dot(X.to_numpy(),beta),observed=y)` | 개별 정상·불량 라벨을 베르누이분포로 연결합니다. |
| 18 | `    idata=pm.sample(900,tune=900,chains=2,cores=1,random_seed=42,progressbar=False)` | MCMC로 사후분포 표본을 추출합니다. |
| 19 | `arr=idata.posterior["beta"].stack(sample=("chain","draw")).values` | 계산 결과나 설정값을 변수에 저장합니다. |
| 20 | `rows=[]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 21 | `for i,f in enumerate(features):` | 여러 그룹 또는 의사결정 조건을 반복 계산합니다. |
| 22 | `    h=az.hdi(arr[i],hdi_prob=.94); rows.append({"feature":f,"mean":arr[i].mean(),"p_positive":(arr[i]>0).mean(),"hdi_low":h[0],"hdi_high":h[1]})` | 최고밀도구간을 계산합니다. |
| 23 | `out=pd.DataFrame(rows); print(out.round(4)); out.to_csv(OUTPUT_DIR/"ex190_beta_probabilities.csv",index=False,encoding="utf-8-sig")` | 결과를 CSV 파일로 저장합니다. |
