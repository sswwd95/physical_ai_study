...
## 1. 학습 목표
Gradient Boosting Regressor로 잔차를 순차적으로 개선합니다.
...
python examples\ex152_gradient_boosting_regressor.py
```

## 4. 예상 결과
Gradient Boosting의 수율 회귀 성능이 출력됩니다.
...
| 16 | `from sklearn.ensemble import GradientBoostingRegressor` | 필요한 라이브러리나 회귀 모델을 불러옵니다. |
...
| 48 | `        "cat",` | 수율 회귀 모델의 학습·평가 단계를 수행합니다. |
| 49 | `        OneHotEncoder(handle_unknown="ignore", sparse_output=False),` | 계산 결과나 설정값을 변수에 저장합니다. |
| 50 | `        categorical_features,` | 수율 회귀 모델의 학습·평가 단계를 수행합니다. |
| 51 | `    ),` | 수율 회귀 모델의 학습·평가 단계를 수행합니다. |
| 52 | `])` | 수율 회귀 모델의 학습·평가 단계를 수행합니다. |
| 53 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 54 | `model = Pipeline([` | 전처리와 회귀 모델을 하나의 파이프라인으로 연결합니다. |
| 55 | `    ("preprocess", preprocessor),` | 수율 회귀 모델의 학습·평가 단계를 수행합니다. |
| 56 | `    (` | 수율 회귀 모델의 학습·평가 단계를 수행합니다. |
| 57 | `        "regressor",` | 수율 회귀 모델의 학습·평가 단계를 수행합니다. |
| 58 | `        GradientBoostingRegressor(` | 수율 회귀 모델의 학습·평가 단계를 수행합니다. |
| 59 | `            n_estimators=250,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 60 | `            learning_rate=0.05,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 61 | `            max_depth=3,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 62 | `            random_state=42,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 63 | `        ),` | 수율 회귀 모델의 학습·평가 단계를 수행합니다. |
| 64 | `    ),` | 수율 회귀 모델의 학습·평가 단계를 수행합니다. |
| 65 | `])` | 수율 회귀 모델의 학습·평가 단계를 수행합니다. |
| 66 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 67 | `model.fit(x_train, y_train)` | 학습 데이터로 전처리기와 회귀 모델을 학습합니다. |
| 68 | `prediction = model.predict(x_test)` | 학습된 모델로 수율을 예측합니다. |
| 69 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 70 | `print("MAE:", round(mean_absolute_error(y_test, prediction), 4))` | 평균 절대 오차를 계산합니다. |
| 71 | `print("RMSE:", round(mean_squared_error(y_test, prediction) ** 0.5, 4))` | 제곱 오차를 이용한 RMSE 계산에 사용합니다. |
| 72 | `print("R2:", round(r2_score(y_test, prediction), 4))` | 모델이 수율 변동을 얼마나 설명하는지 R²를 계산합니다. |
