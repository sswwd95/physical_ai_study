...
## 1. 학습 목표
Decision Tree Regressor로 비선형 수율 관계를 학습합니다.
...
python examples\ex149_decision_tree_regressor.py
```

## 4. 예상 결과
비선형 Decision Tree 수율 예측 성능이 출력됩니다.
...
| 16 | `from sklearn.metrics import mean_absolute_error, mean_squared_error, r2_score` | 필요한 라이브러리나 회귀 모델을 불러옵니다. |
...
| 20 | `from sklearn.tree import DecisionTreeRegressor` | 필요한 라이브러리나 회귀 모델을 불러옵니다. |
...
| 54 | `        DecisionTreeRegressor(` | 수율 회귀 모델의 학습·평가 단계를 수행합니다. |
| 55 | `            max_depth=6,` | 계산 결과나 설정값을 변수에 저장합니다. |
...
| 65 | `print("MAE:", round(mean_absolute_error(y_test, prediction), 4))` | 평균 절대 오차를 계산합니다. |
| 66 | `print("RMSE:", round(mean_squared_error(y_test, prediction) ** 0.5, 4))` | 제곱 오차를 이용한 RMSE 계산에 사용합니다. |
| 67 | `print("R2:", round(r2_score(y_test, prediction), 4))` | 모델이 수율 변동을 얼마나 설명하는지 R²를 계산합니다. |
