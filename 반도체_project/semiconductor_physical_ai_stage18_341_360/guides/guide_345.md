# 실습 345 — safety_state_machine

## 1. 학습 목표
정상·주의·감속·정지 상태기계를 구현합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
NORMAL·CAUTION·SLOWDOWN·STOP 상태기계를 작성하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage18
python examples\ex345_safety_state_machine.py
```
...
| 14 | `safe_df["safety_state"]=np.select(` | 여러 안전 조건에 따라 행동을 선택합니다. |
| 15 | `    [` | 이상 대응 또는 안전 의사결정 단계를 수행합니다. |
| 16 | `        (safe_df["door_closed"]==0)\|(safe_df["cooling_ok"]==0)\|(safe_df["vacuum_ok"]==0)\|(safe_df["severity_level"]>=4),` | 계산 결과나 안전 설정값을 변수에 저장합니다. |
| 17 | `        safe_df["severity_level"]==3,` | 계산 결과나 안전 설정값을 변수에 저장합니다. |
| 18 | `        safe_df["severity_level"].between(1,2)` | 이상 대응 또는 안전 의사결정 단계를 수행합니다. |
| 19 | `    ],` | 이상 대응 또는 안전 의사결정 단계를 수행합니다. |
| 20 | `    ["STOP","SLOWDOWN","CAUTION"],` | 이상 대응 또는 안전 의사결정 단계를 수행합니다. |
| 21 | `    default="NORMAL")` | 계산 결과나 안전 설정값을 변수에 저장합니다. |
| 22 | `print(safe_df["safety_state"].value_counts())` | 결과를 콘솔에 출력합니다. |
