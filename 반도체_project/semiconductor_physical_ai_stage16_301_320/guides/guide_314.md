...
python examples\ex314_sensor_fault_detection.py
```
...
| 17 | `sensor_df["temp_b_residual"] = (` | 계산 결과나 설정값을 변수에 저장합니다. |
...
| 21 | `sensor_df["pressure_b_residual"] = (` | 계산 결과나 설정값을 변수에 저장합니다. |
| 22 | `    sensor_df["pressure_sensor_b_pa"]` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
| 23 | `    - sensor_df["true_pressure_pa"]` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
| 24 | `)` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
| 25 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 26 | `sensor_df["temp_sensor_fault"] = (` | 계산 결과나 설정값을 변수에 저장합니다. |
| 27 | `    sensor_df["temp_b_residual"].abs() > 2.0` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
| 28 | `)` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
| 29 | `sensor_df["pressure_sensor_fault"] = (` | 계산 결과나 설정값을 변수에 저장합니다. |
| 30 | `    sensor_df["pressure_b_residual"].abs() > 1.0` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
| 31 | `)` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
| 32 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 33 | `print(` | 결과를 콘솔에 출력합니다. |
| 34 | `    sensor_df[` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
| 35 | `        ["temp_sensor_fault", "pressure_sensor_fault"]` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
| 36 | `    ].sum()` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
| 37 | `)` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
| 38 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 39 | `sensor_df.loc[` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
| 40 | `    sensor_df["temp_sensor_fault"]` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
| 41 | `    \| sensor_df["pressure_sensor_fault"]` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
| 42 | `].to_csv(` | 결과를 CSV 파일로 저장합니다. |
| 43 | `    OUTPUT_DIR / "ex314_sensor_fault_rows.csv",` | 디지털 트윈·센서 융합 분석 단계를 수행합니다. |
