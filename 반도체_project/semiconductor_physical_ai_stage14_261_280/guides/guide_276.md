...
```text
각 조건 선택의 기대 후회를 계산하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage14
python examples\ex276_expected_regret.py
```
...
| 16 | `draws=np.random.default_rng(42).normal(mu,se,size=(5000,len(group)))` | 계산 결과나 모형 요소를 변수에 저장합니다. |
| 17 | `best=draws.max(axis=1,keepdims=True)` | 계산 결과나 모형 요소를 변수에 저장합니다. |
| 18 | `regret=(best-draws).mean(axis=0)` | 계산 결과나 모형 요소를 변수에 저장합니다. |
| 19 | `group["expected_regret"]=regret` | 계산 결과나 모형 요소를 변수에 저장합니다. |
| 20 | `out=group.sort_values("expected_regret")` | 계산 결과나 모형 요소를 변수에 저장합니다. |
| 21 | `print(out.head(10).round(4))` | 결과를 콘솔에 출력합니다. |
| 22 | `out.to_csv(OUTPUT_DIR/"ex276_expected_regret.csv",index=False,encoding="utf-8-sig")` | 결과를 CSV로 저장합니다. |
