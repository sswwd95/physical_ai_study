# 실습 267 — chamber_block_effect

## 1. 학습 목표
챔버 블록 효과를 포함한 품질 비교를 수행합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
레시피 효과와 챔버 블록 효과를 동시에 추정하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage14
python examples\ex267_chamber_block_effect.py
```
...
| 18 | `    recipe_eff=pm.Normal("recipe_eff",0,1,dims="recipe")` | 평균·효과·회귀계수에 정규 사전분포를 정의합니다. |
| 19 | `    chamber_eff=pm.Normal("chamber_eff",0,1,dims="chamber")` | 평균·효과·회귀계수에 정규 사전분포를 정의합니다. |
| 20 | `    sigma=pm.HalfNormal("sigma",2)` | 0보다 큰 표준편차 파라미터를 정의합니다. |
| 21 | `    mu=a+recipe_eff[r_codes]+chamber_eff[c_codes]` | 계산 결과나 모형 요소를 변수에 저장합니다. |
| 22 | `    pm.Normal("y",mu,sigma,observed=experiment_df["uniformity_percent"])` | 평균·효과·회귀계수에 정규 사전분포를 정의합니다. |
| 23 | `    idata=pm.sample(1000,tune=1000,chains=2,cores=1,random_seed=42,progressbar=False)` | MCMC로 사후분포 표본을 추출합니다. |
| 24 | `print(az.summary(idata,var_names=["recipe_eff","chamber_eff"],hdi_prob=.94))` | 사후요약과 MCMC 진단값을 계산합니다. |
