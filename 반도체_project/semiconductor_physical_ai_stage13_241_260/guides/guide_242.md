...
python examples\ex242_exponential_lifetime_model.py
```
...
| 18 | `    rate = pm.Exponential("rate", 1/120)` | 양수 파라미터에 지수분포 사전분포를 사용합니다. |
| 19 | `    pm.Exponential("lifetime", lam=rate, observed=observed)` | 양수 파라미터에 지수분포 사전분포를 사용합니다. |
| 20 | `    idata = pm.sample(1000,tune=1000,chains=2,cores=1,random_seed=42,progressbar=False)` | MCMC로 사후분포 표본을 추출합니다. |
| 21 | `summary=az.summary(idata,var_names=["rate"],hdi_prob=.94)` | 사후요약과 R-hat·ESS를 계산합니다. |
| 22 | `print(summary)` | 결과를 콘솔에 출력합니다. |
