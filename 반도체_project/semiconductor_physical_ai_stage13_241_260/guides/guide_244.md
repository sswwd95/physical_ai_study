...
python examples\ex244_censored_weibull_model.py
```
...
| 16 | `time=life_df["observed_cycles"].to_numpy()` | 계산 결과나 모형 파라미터를 변수에 저장합니다. |
| 17 | `upper=np.where(life_df["event_observed"].to_numpy()==1,time,np.inf)` | 계산 결과나 모형 파라미터를 변수에 저장합니다. |
| 18 | `with pm.Model() as model:` | PyMC 확률모형 범위를 시작합니다. |
| 19 | `    alpha=pm.HalfNormal("alpha",3)` | 0보다 큰 분산·스케일 파라미터를 정의합니다. |
| 20 | `    beta=pm.HalfNormal("beta",150)` | 0보다 큰 분산·스케일 파라미터를 정의합니다. |
| 21 | `    base=pm.Weibull.dist(alpha=alpha,beta=beta)` | 설비 수명을 Weibull 분포로 모델링합니다. |
| 22 | `    pm.Censored("lifetime",base,lower=None,upper=upper,observed=time)` | 우측 검열된 설비 수명 데이터를 관측모형에 반영합니다. |
| 23 | `    idata=pm.sample(1000,tune=1000,chains=2,cores=1,random_seed=42,progressbar=False,target_accept=.9)` | MCMC로 사후분포 표본을 추출합니다. |
| 24 | `print(az.summary(idata,var_names=["alpha","beta"],hdi_prob=.94))` | 사후요약과 R-hat·ESS를 계산합니다. |
