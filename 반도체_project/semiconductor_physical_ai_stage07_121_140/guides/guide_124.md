...
## 1. 학습 목표
다항 Logistic Regression으로 기본 다중 클래스 모델을 학습합니다.
...
python examples\ex124_multinomial_logistic_regression.py
```

## 4. 예상 결과
다항 Logistic Regression의 macro·weighted F1이 출력됩니다.
...
| 52 | `        LogisticRegression(` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
| 53 | `            max_iter=2000,` | 계산 결과나 설정값을 변수에 저장합니다. |
...
| 60 | `model.fit(x_train, y_train)` | 학습 데이터로 전처리기와 모델을 학습합니다. |
| 61 | `y_pred = model.predict(x_test)` | 학습된 모델로 불량 유형을 예측합니다. |
| 62 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 63 | `print("Macro F1:", round(f1_score(y_test, y_pred, average="macro"), 4))` | 실행 결과를 콘솔에 출력합니다. |
| 64 | `print("Weighted F1:", round(f1_score(y_test, y_pred, average="weighted"), 4))` | 실행 결과를 콘솔에 출력합니다. |
