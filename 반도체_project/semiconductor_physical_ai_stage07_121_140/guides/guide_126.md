...
## 1. 학습 목표
Decision Tree로 다중 불량 유형의 비선형 분기 규칙을 학습합니다.
...
python examples\ex126_multiclass_decision_tree.py
```

## 4. 예상 결과
Decision Tree의 다중 클래스 성능과 혼동행렬이 출력됩니다.
...
| 16 | `from sklearn.metrics import confusion_matrix, f1_score` | 필요한 라이브러리나 모델을 불러옵니다. |
...
| 52 | `        DecisionTreeClassifier(` | 다중 클래스 분류 또는 모델 튜닝 단계를 수행합니다. |
| 53 | `            max_depth=6,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 54 | `            min_samples_leaf=8,` | 계산 결과나 설정값을 변수에 저장합니다. |
...
| 64 | `print("Macro F1:", round(f1_score(y_test, y_pred, average="macro"), 4))` | 실행 결과를 콘솔에 출력합니다. |
| 65 | `print("Weighted F1:", round(f1_score(y_test, y_pred, average="weighted"), 4))` | 실행 결과를 콘솔에 출력합니다. |
| 66 | `print("혼동행렬:")` | 실행 결과를 콘솔에 출력합니다. |
| 67 | `print(confusion_matrix(y_test, y_pred, labels=model.classes_))` | 실제 클래스와 예측 클래스의 조합을 표로 계산합니다. |
