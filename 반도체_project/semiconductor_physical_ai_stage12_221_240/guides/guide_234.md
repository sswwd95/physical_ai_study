...
python examples\ex234_rul_residual_analysis.py
```
...
| 21 | `m=RandomForestRegressor(n_estimators=350,random_state=42,n_jobs=-1); m.fit(X.iloc[tr],y.iloc[tr]); p=m.predict(X.iloc[te])` | 학습 데이터로 모델을 학습합니다. |
| 22 | `out=pm_df.iloc[te][["equipment_id","cycle","rul_cycles"]].copy(); out["predicted_rul"]=p; out["residual"]=out["rul_cycles"]-out["predicted_rul"]; out["absolute_error"]=out["residual"].abs()` | 계산 결과나 설정값을 변수에 저장합니다. |
| 23 | `print(out.sort_values("absolute_error",ascending=False).head(20).round(3))` | 결과를 콘솔에 출력합니다. |
| 24 | `out.to_csv(OUTPUT_DIR/"ex234_rul_residuals.csv",index=False,encoding="utf-8-sig")` | 결과를 CSV로 저장합니다. |
