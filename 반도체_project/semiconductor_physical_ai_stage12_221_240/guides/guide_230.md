...
python examples\ex230_rul_linear_regression.py
```
...
| 19 | `from sklearn.linear_model import LinearRegression` | 필요한 라이브러리나 모델을 불러옵니다. |
...
| 22 | `X=pm_df[features]; y=pm_df["rul_cycles"]; groups=pm_df["equipment_id"]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 23 | `tr,te=next(GroupShuffleSplit(n_splits=1,test_size=.25,random_state=42).split(X,y,groups))` | 학습과 평가 데이터를 분리합니다. |
| 24 | `model=Pipeline([("scale",StandardScaler()),("regressor",LinearRegression())])` | 계산 결과나 설정값을 변수에 저장합니다. |
| 25 | `model.fit(X.iloc[tr],y.iloc[tr]); pred=model.predict(X.iloc[te])` | 학습 데이터로 모델을 학습합니다. |
| 26 | `print("MAE:",round(mean_absolute_error(y.iloc[te],pred),4))` | 결과를 콘솔에 출력합니다. |
| 27 | `print("RMSE:",round(mean_squared_error(y.iloc[te],pred)**.5,4))` | 결과를 콘솔에 출력합니다. |
| 28 | `print("R2:",round(r2_score(y.iloc[te],pred),4))` | 결과를 콘솔에 출력합니다. |
