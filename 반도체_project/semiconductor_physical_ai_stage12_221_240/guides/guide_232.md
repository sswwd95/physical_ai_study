...
## 1. 학습 목표
Gradient Boosting으로 잔여수명을 예측합니다.
...
python examples\ex232_rul_gradient_boosting.py
```
...
| 17 | `from sklearn.ensemble import GradientBoostingRegressor` | 필요한 라이브러리나 모델을 불러옵니다. |
...
| 22 | `model=GradientBoostingRegressor(n_estimators=250,learning_rate=.05,max_depth=3,random_state=42)` | 계산 결과나 설정값을 변수에 저장합니다. |
