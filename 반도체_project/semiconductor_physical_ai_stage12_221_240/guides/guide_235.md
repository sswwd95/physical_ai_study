...
python examples\ex235_near_failure_error.py
```
...
| 16 | `from sklearn.metrics import mean_absolute_error` | 필요한 라이브러리나 모델을 불러옵니다. |
...
| 22 | `m=RandomForestRegressor(n_estimators=350,random_state=42,n_jobs=-1); m.fit(X.iloc[tr],y.iloc[tr]); p=m.predict(X.iloc[te])` | 학습 데이터로 모델을 학습합니다. |
| 23 | `mask=y.iloc[te]<=20` | 계산 결과나 설정값을 변수에 저장합니다. |
| 24 | `print("전체 MAE:",round(mean_absolute_error(y.iloc[te],p),4))` | 결과를 콘솔에 출력합니다. |
| 25 | `print("고장임박 MAE:",round(mean_absolute_error(y.iloc[te][mask],p[mask]),4))` | 결과를 콘솔에 출력합니다. |
