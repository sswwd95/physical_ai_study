...
## 1. 학습 목표
이동평균과 이동표준편차로 공정의 국소 추세와 변동성을 계산합니다.
...
python examples\ex043_rolling_mean_std.py
```

## 4. 예상 결과
최근 20개 시점의 온도 평균과 표준편차가 시점별로 계산됩니다.
...
| 15 | `sensor_df["temp_rolling_mean_20"] = (` | 오른쪽 계산 결과를 왼쪽 변수에 저장합니다. |
| 16 | `    sensor_df["chamber_temp_c"]` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 17 | `    .rolling(window=20, min_periods=5)` | 지정한 구간의 이동통계를 계산합니다. |
| 18 | `    .mean()` | 데이터의 평균을 계산합니다. |
| 19 | `)` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 20 | `sensor_df["temp_rolling_std_20"] = (` | 오른쪽 계산 결과를 왼쪽 변수에 저장합니다. |
| 21 | `    sensor_df["chamber_temp_c"]` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 22 | `    .rolling(window=20, min_periods=5)` | 지정한 구간의 이동통계를 계산합니다. |
| 23 | `    .std()` | 데이터의 표준편차를 계산합니다. |
| 24 | `)` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 25 | `` | 코드 구역을 보기 좋게 나누는 빈 줄입니다. |
| 26 | `result_df = sensor_df[` | 오른쪽 계산 결과를 왼쪽 변수에 저장합니다. |
| 27 | `    [` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 28 | `        "timestamp",` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 29 | `        "chamber_temp_c",` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 30 | `        "temp_rolling_mean_20",` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 31 | `        "temp_rolling_std_20",` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 32 | `    ]` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 33 | `]` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
| 34 | `` | 코드 구역을 보기 좋게 나누는 빈 줄입니다. |
| 35 | `print(result_df.tail(30).round(3))` | 실행 결과를 콘솔에 출력합니다. |
| 36 | `result_df.to_csv(` | 계산 결과를 CSV 파일로 저장합니다. |
| 37 | `    OUTPUT_DIR / "ex043_rolling_mean_std.csv",` | 공정 모니터링 또는 SPC 계산 단계를 수행합니다. |
