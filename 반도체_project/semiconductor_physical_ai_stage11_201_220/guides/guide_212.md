# 실습 212 — threshold_review_policy

## 1. 학습 목표
고장확률 임계값과 재검사 정책을 비교합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
고장 확률 임계값과 재검사 구간을 비교하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage11
python examples\ex212_threshold_review_policy.py
```
...
| 22 | `X=sensor_df[features]; y=(sensor_df["fault_type"]!="normal").astype(int)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 23 | `Xtr,Xte,ytr,yte=train_test_split(X,y,test_size=.25,random_state=42,stratify=y)` | 학습용 데이터와 평가용 데이터를 분리합니다. |
...
| 25 | `m=Pipeline([("preprocess",pre),("classifier",RandomForestClassifier(n_estimators=300,class_weight="balanced",random_state=42,n_jobs=-1))])` | 전처리와 고장 분류 모델을 하나로 연결합니다. |
| 26 | `m.fit(Xtr,ytr); prob=m.predict_proba(Xte)[:,1]` | 학습 데이터로 모델을 학습합니다. |
| 27 | `rows=[]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 28 | `for t in [.3,.4,.5,.6,.7]:` | 여러 모델이나 설정에 같은 작업을 반복합니다. |
| 29 | `    pred=(prob>=t).astype(int)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 30 | `    rows.append({"threshold":t,"predicted_faults":int(pred.sum()),"review_count":int(((prob>=t-.1)&(prob<t)).sum())})` | 계산 결과나 설정값을 변수에 저장합니다. |
| 31 | `out=pd.DataFrame(rows); print(out); out.to_csv(OUTPUT_DIR/"ex212_threshold_policy.csv",index=False,encoding="utf-8-sig")` | 결과를 CSV 파일로 저장합니다. |
