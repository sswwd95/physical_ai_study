# 실습 214 — classwise_metrics

## 1. 학습 목표
클래스별 precision·recall·F1을 저장합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
classification_report를 DataFrame으로 저장하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage11
python examples\ex214_classwise_metrics.py
```
...
| 17 | `from sklearn.metrics import classification_report` | 필요한 라이브러리나 분류 모델을 불러옵니다. |
...
| 27 | `m.fit(Xtr,ytr); pred=m.predict(Xte)` | 학습 데이터로 모델을 학습합니다. |
| 28 | `out=pd.DataFrame(classification_report(yte,pred,output_dict=True,zero_division=0)).T` | 클래스별 정밀도·재현율·F1을 요약합니다. |
| 29 | `print(out.round(4)); out.to_csv(OUTPUT_DIR/"ex214_classwise_metrics.csv",encoding="utf-8-sig")` | 결과를 CSV 파일로 저장합니다. |
