# 실습 205 — logistic_fault_classifier

## 1. 학습 목표
Logistic Regression으로 정상·고장을 분류합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
Logistic Regression으로 정상·고장 이진 분류를 수행하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage11
python examples\ex205_logistic_fault_classifier.py
```
...
| 20 | `from sklearn.linear_model import LogisticRegression` | 필요한 라이브러리나 분류 모델을 불러옵니다. |
...
| 27 | `X=sensor_df[features]; y=(sensor_df["fault_type"]!="normal").astype(int)` | 계산 결과나 설정값을 변수에 저장합니다. |
...
| 32 | `    ("num",StandardScaler(),num),` | 장비 상태 진단 또는 고장 분류 단계를 수행합니다. |
...
| 36 | `    ("classifier",LogisticRegression(max_iter=1500,class_weight="balanced",random_state=42))])` | 계산 결과나 설정값을 변수에 저장합니다. |
| 37 | `model.fit(X_train,y_train)` | 학습 데이터로 모델을 학습합니다. |
| 38 | `pred=model.predict(X_test)` | 장비 상태 또는 고장 유형을 예측합니다. |
| 39 | `print(classification_report(y_test,pred,zero_division=0))` | 클래스별 정밀도·재현율·F1을 요약합니다. |
