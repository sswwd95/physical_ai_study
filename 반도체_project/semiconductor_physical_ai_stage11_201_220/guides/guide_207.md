# 실습 207 — decision_tree_faults

## 1. 학습 목표
Decision Tree로 고장 규칙을 학습합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
Decision Tree로 고장 유형을 분류하고 성능을 출력하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage11
python examples\ex207_decision_tree_faults.py
```
...
| 20 | `from sklearn.tree import DecisionTreeClassifier` | 필요한 라이브러리나 분류 모델을 불러옵니다. |
...
| 36 | `    ("classifier",DecisionTreeClassifier(max_depth=7,min_samples_leaf=8,class_weight="balanced",random_state=42))])` | 계산 결과나 설정값을 변수에 저장합니다. |
