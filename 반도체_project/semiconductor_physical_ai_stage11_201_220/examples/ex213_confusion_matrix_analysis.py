...
m.fit(Xtr,ytr); pred=m.predict(Xte); labels=m.classes_
mat=pd.DataFrame(confusion_matrix(yte,pred,labels=labels),index=[f"actual_{x}" for x in labels],columns=[f"pred_{x}" for x in labels])
print(mat); mat.to_csv(OUTPUT_DIR/"ex213_confusion_matrix.csv",encoding="utf-8-sig")
