...
m.fit(X.iloc[tr],y.iloc[tr]); prob=m.predict_proba(X.iloc[te]); pred=m.predict(X.iloc[te]); classes=m.classes_
out=sensor_df.iloc[te][["timestamp","equipment_id","operation_mode","fault_type"]].copy(); out=out.rename(columns={"fault_type":"actual_fault"}); out["predicted_fault"]=pred; out["max_probability"]=prob.max(1)
for i,c in enumerate(classes): out[f"probability_{c}"]=prob[:,i]
out=out.sort_values("max_probability",ascending=False); print(out.head(15).round(4)); out.to_csv(OUTPUT_DIR/"ex219_fault_predictions.csv",index=False,encoding="utf-8-sig")
