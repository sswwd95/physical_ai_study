...
```text
범주형 전처리를 포함한 RandomForest 고장확률 모델을 작성하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage20
python examples\ex387_fault_model_training.py
```
...
| 18 | `from sklearn.ensemble import RandomForestClassifier` | 필요한 라이브러리나 모델을 불러옵니다. |
| 19 | `from sklearn.metrics import classification_report` | 필요한 라이브러리나 모델을 불러옵니다. |
...
| 25 | `X=data[features]; y=data["fault_flag"]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 26 | `Xtr,Xte,ytr,yte=train_test_split(X,y,test_size=.25,random_state=42,stratify=y)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 27 | `pre=ColumnTransformer([("num","passthrough",features[2:]),("cat",OneHotEncoder(handle_unknown="ignore"),features[:2])])` | 계산 결과나 설정값을 변수에 저장합니다. |
| 28 | `model=Pipeline([("preprocess",pre),("classifier",RandomForestClassifier(n_estimators=350,class_weight="balanced",random_state=42,n_jobs=-1))])` | 계산 결과나 설정값을 변수에 저장합니다. |
| 29 | `model.fit(Xtr,ytr); pred=model.predict(Xte)` | 학습 데이터로 모델을 학습합니다. |
| 30 | `print(classification_report(yte,pred,zero_division=0))` | 결과를 콘솔에 출력합니다. |
