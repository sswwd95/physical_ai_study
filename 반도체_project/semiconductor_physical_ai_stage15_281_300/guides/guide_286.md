# 실습 286 — surrogate_models

## 1. 학습 목표
균일도·불량률·시간 대리모델을 학습합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
균일도·불량률·사이클타임 대리모델의 학습점수를 비교하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage15
python examples\ex286_surrogate_models.py
```
...
| 40 | `targets=["uniformity_percent","defect_rate","cycle_time_min"]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 41 | `rows=[]` | 계산 결과나 설정값을 변수에 저장합니다. |
| 42 | `for target in targets:` | 여러 후보나 가중치 조합을 반복 계산합니다. |
| 43 | `    model=make_model()` | 계산 결과나 설정값을 변수에 저장합니다. |
| 44 | `    model.fit(history_df[features],history_df[target])` | 과거 공정 데이터로 대리모델을 학습합니다. |
| 45 | `    rows.append({"target":target,"train_score":model.score(history_df[features],history_df[target])})` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 46 | `out=pd.DataFrame(rows)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 47 | `print(out.round(4))` | 결과를 콘솔에 출력합니다. |
| 48 | `out.to_csv(OUTPUT_DIR/"ex286_surrogate_scores.csv",index=False,encoding="utf-8-sig")` | 추천 결과를 CSV로 저장합니다. |
