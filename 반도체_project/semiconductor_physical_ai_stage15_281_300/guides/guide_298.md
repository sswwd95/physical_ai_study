# 실습 298 — cost_sensitive_decision

## 1. 학습 목표
품질·불량·시간·실험비용을 결합합니다.

## 2. Antigravity용 예제 소스 생성 하네스 프롬프트
```text
품질손실·불량비용·시간비용·실험비용을 합산하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage15
python examples\ex298_cost_sensitive_decision.py
```
...
| 44 | `candidate_df["expected_quality_loss"]=(97-candidate_df["uniformity_percent"]).clip(lower=0)*500` | 계산 결과나 설정값을 변수에 저장합니다. |
| 45 | `candidate_df["expected_defect_cost"]=candidate_df["defect_rate"]*100*200` | 계산 결과나 설정값을 변수에 저장합니다. |
| 46 | `candidate_df["time_cost"]=candidate_df["cycle_time_min"]*8` | 계산 결과나 설정값을 변수에 저장합니다. |
| 47 | `candidate_df["experiment_cost"]=150` | 계산 결과나 설정값을 변수에 저장합니다. |
| 48 | `candidate_df["total_expected_cost"]=(` | 계산 결과나 설정값을 변수에 저장합니다. |
| 49 | `    candidate_df["expected_quality_loss"]` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 50 | `    +candidate_df["expected_defect_cost"]` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 51 | `    +candidate_df["time_cost"]` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 52 | `    +candidate_df["experiment_cost"]` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 53 | `)` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 54 | `out=candidate_df.sort_values("total_expected_cost").head(25)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 55 | `print(out.round(2))` | 결과를 콘솔에 출력합니다. |
| 56 | `out.to_csv(OUTPUT_DIR/"ex298_cost_sensitive_decision.csv",index=False,encoding="utf-8-sig")` | 추천 결과를 CSV로 저장합니다. |
