# 실습 292 — probability_of_improvement
...
```text
현재 최고 균일도 초과확률을 계산하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage15
python examples\ex292_probability_of_improvement.py
```
...
| 49 | `candidate_df["probability_improvement"]=norm.cdf((mean-best)/std)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 50 | `out=candidate_df.sort_values("probability_improvement",ascending=False).head(25)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 51 | `print(out.round(6))` | 결과를 콘솔에 출력합니다. |
