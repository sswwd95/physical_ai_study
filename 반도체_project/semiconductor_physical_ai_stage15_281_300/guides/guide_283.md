# 실습 283 — single_objective_defect_rate
...
python examples\ex283_single_objective_defect_rate.py
```
...
| 41 | `model.fit(history_df[features],history_df["defect_rate"])` | 과거 공정 데이터로 대리모델을 학습합니다. |
| 42 | `candidate_df["predicted_defect_rate"]=model.predict(candidate_df[features])` | 후보 공정 조건의 품질·불량률·시간을 예측합니다. |
| 43 | `out=candidate_df.sort_values("predicted_defect_rate").head(20)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 44 | `print(out.round(5))` | 결과를 콘솔에 출력합니다. |
| 45 | `out.to_csv(OUTPUT_DIR/"ex283_defect_optimum.csv",index=False,encoding="utf-8-sig")` | 추천 결과를 CSV로 저장합니다. |
