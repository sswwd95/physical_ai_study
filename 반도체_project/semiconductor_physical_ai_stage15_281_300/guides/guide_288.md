...
```text
균일도 55%, 불량률 30%, 시간 15% 가중 효용을 계산하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage15
python examples\ex288_weighted_utility.py
```
...
| 44 | `def zscore(series):` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 45 | `    return (series-series.mean())/series.std()` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 46 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 47 | `candidate_df["utility"]=(` | 계산 결과나 설정값을 변수에 저장합니다. |
| 48 | `    0.55*zscore(candidate_df["uniformity_percent"])` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 49 | `    -0.30*zscore(candidate_df["defect_rate"])` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 50 | `    -0.15*zscore(candidate_df["cycle_time_min"])` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 51 | `)` | 공정 최적화 또는 베이지안 의사결정 단계를 수행합니다. |
| 52 | `out=candidate_df.sort_values("utility",ascending=False).head(30)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 53 | `print(out.round(4))` | 결과를 콘솔에 출력합니다. |
| 54 | `out.to_csv(OUTPUT_DIR/"ex288_weighted_utility.csv",index=False,encoding="utf-8-sig")` | 추천 결과를 CSV로 저장합니다. |
