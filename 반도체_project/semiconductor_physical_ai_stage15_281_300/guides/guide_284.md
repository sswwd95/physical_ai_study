# 실습 284 — cycle_time_optimization
...
```text
사이클타임 최소 후보 20개를 선택하라.
```

## 3. 실행 명령
```bat
conda activate semi-physical-ai-stage15
python examples\ex284_cycle_time_optimization.py
```
...
| 41 | `model.fit(history_df[features],history_df["cycle_time_min"])` | 과거 공정 데이터로 대리모델을 학습합니다. |
| 42 | `candidate_df["predicted_cycle_time"]=model.predict(candidate_df[features])` | 후보 공정 조건의 품질·불량률·시간을 예측합니다. |
| 43 | `out=candidate_df.sort_values("predicted_cycle_time").head(20)` | 계산 결과나 설정값을 변수에 저장합니다. |
| 44 | `print(out.round(4))` | 결과를 콘솔에 출력합니다. |
| 45 | `out.to_csv(OUTPUT_DIR/"ex284_cycle_time_optimum.csv",index=False,encoding="utf-8-sig")` | 추천 결과를 CSV로 저장합니다. |
