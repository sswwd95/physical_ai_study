...
## 1. 학습 목표
전처리 파이프라인과 Logistic Regression으로 첫 불량 분류 모델을 학습합니다.
...
python examples\ex106_logistic_regression_basic.py
```

## 4. 예상 결과
Logistic Regression의 기본 분류 성능이 출력됩니다.
...
| 42 | `    x,` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 43 | `    y,` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 44 | `    test_size=0.25,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 45 | `    random_state=42,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 46 | `    stratify=y,` | 계산 결과나 설정값을 변수에 저장합니다. |
| 47 | `)` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 48 | `` | 코드 구역을 구분하는 빈 줄입니다. |
| 49 | `preprocessor = ColumnTransformer([` | 숫자형과 범주형 컬럼의 전처리를 하나로 묶습니다. |
| 50 | `    ("num", StandardScaler(), numeric_features),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
...
| 54 | `model = Pipeline([` | 전처리와 모델 학습 과정을 하나의 파이프라인으로 연결합니다. |
| 55 | `    ("preprocess", preprocessor),` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 56 | `    (` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 57 | `        "classifier",` | 분류 모델의 전처리·학습·평가 단계를 수행합니다. |
| 58 | `        LogisticRegression(max_iter=1000, random_state=42),` | 계산 결과나 설정값을 변수에 저장합니다. |
