...
python examples\ex323_bayesian_temperature_fusion.py
```
...
| 17 | `    latent=pm.Normal("latent",mu=50,sigma=30,dims="obs")` | 센서 바이어스·참값·관측값의 정규분포를 정의합니다. |
| 18 | `    bias_a=pm.Normal("bias_a",0,2); bias_b=pm.Normal("bias_b",0,2)` | 센서 바이어스·참값·관측값의 정규분포를 정의합니다. |
| 19 | `    sigma_a=pm.HalfNormal("sigma_a",1); sigma_b=pm.HalfNormal("sigma_b",1)` | 0보다 큰 센서 노이즈 표준편차를 정의합니다. |
...
| 23 | `mean=idata.posterior["latent"].mean(("chain","draw")).values` | 계산 결과나 모형 파라미터를 변수에 저장합니다. |
| 24 | `print("융합 온도 처음 10개:",np.round(mean[:10],3))` | 결과를 콘솔에 출력합니다. |
