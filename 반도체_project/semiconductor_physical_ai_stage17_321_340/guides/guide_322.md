...
python examples\ex322_sensor_noise_posterior.py
```
...
| 20 | `    pm.Normal("a",bias_a,sigma_a,observed=res_a)` | 센서 바이어스·참값·관측값의 정규분포를 정의합니다. |
| 21 | `    pm.Normal("b",bias_b,sigma_b,observed=res_b)` | 센서 바이어스·참값·관측값의 정규분포를 정의합니다. |
| 22 | `    idata=pm.sample(1000,tune=1000,chains=2,cores=1,random_seed=42,progressbar=False)` | MCMC로 사후분포 표본을 추출합니다. |
| 23 | `print(az.summary(idata,var_names=["sigma_a","sigma_b"],hdi_prob=.94))` | 사후요약과 MCMC 진단값을 계산합니다. |
